...
def _plotConnCalculateFromSim(
    ...
    if sim.cfg.compactConnFormat:
        ...
        missing = []
        preGidIndex = (
            connsFormat.index("preGid")
            if "preGid" in connsFormat
            else missing.append("preGid")
        )
        synMechIndex = (
            connsFormat.index("synMech")
            if "synMech" in connsFormat
            else missing.append("synMech")
        )
        weightIndex = (
            connsFormat.index("weight")
            if "weight" in connsFormat
            else missing.append("weight")
        )
        delayIndex = (
            connsFormat.index("delay")
            if "delay" in connsFormat
            else missing.append("delay")
        )
        preLabelIndex = (
            connsFormat.index("preLabel") if "preLabel" in connsFormat else -1
        )
    ...
    if groupBy == "cell":
        if feature in ["weight", "delay", "numConns"]:
            ...
            print(
                '  Conn matrix with groupBy="cell" only supports features= "weight", "delay" or "numConns"'
            )
            return None, None, None
        ...
        if len(cellsPre) > 0 and len(cellsPost) > 0:
            if (
                orderBy not in cellsPre[0]["tags"]
                or orderBy not in cellsPost[0]["tags"]
            ...
            sortedGidsPre = {
                gid: i for i, (y, gid) in enumerate(sorted(zip(yorderPre, cellGidsPre)))
            }
            ...
            if includePre == includePost:
                ...
                sortedGidsPost = {
                    gid: i
                    for i, (y, gid) in enumerate(sorted(zip(yorderPost, cellGidsPost)))
                }
        ...
        for cell in cellsPost:  # for each postsyn cell
            ...
            if synMech:
                cellConns = [
                    conn for conn in cellConns if conn[synMechIndex] in synMech
                ]
            ...
            for conn in cellConns:
                if conn[preGidIndex] != "NetStim" and conn[preGidIndex] in cellIndsPre:
                    if feature in ["weight", "delay"]:
                        featureIndex = (
                            weightIndex if feature == "weight" else delayIndex
                        )
                        if conn[preGidIndex] in cellIndsPre:
                            if removeWeightNorm and feature == "weight":
                                try:
                                    ...
                                    weightNorm = cell["secs"][sec]["weightNorm"][
                                        segIndex
                                    ]
                                    connMatrix[
                                        cellIndsPre[conn[preGidIndex]],
                                        cellIndsPost[cell["gid"]],
                                    ] += (
                    ...
                    countMatrix[
                        cellIndsPre[conn[preGidIndex]], cellIndsPost[cell["gid"]]
                    ] += 1
    ...
    # Calculate matrix if grouped by pop
        ...
        popsPre = [
            pop for pop in sim.net.allPops if pop in popsTempPre
        ] + netStimPopsPre
        ...
        if includePre == includePost:
            ...
            popsPost = [
                pop for pop in sim.net.allPops if pop in popsTempPost
            ] + netStimPopsPost
        ...
        for pop in popsPre:
            if pop in netStimPopsPre:
                ...
                numCellsPopPre[pop] = len(
                    [cell for cell in cellsPre if cell["tags"]["pop"] == pop]
                )
        ...
        if includePre == includePost:
            ...
            for pop in popsPost:
                if pop in netStimPopsPost:
                    ...
                    numCellsPopPost[pop] = len(
                        [cell for cell in cellsPost if cell["tags"]["pop"] == pop]
                    )
        ...
        for prePop in popsPre:
            for postPop in popsPost:
                ...
                if feature == "convergence":
                    maxPostConnMatrix[
                        popIndsPre[prePop], popIndsPost[postPop]
                    ] = numCellsPopPost[postPop]
                if feature == "divergence":
                    maxPreConnMatrix[
                        popIndsPre[prePop], popIndsPost[postPop]
                    ] = numCellsPopPre[prePop]
        ...
        for cell in cellsPost:  # for each postsyn cell
            ...
            for conn in cellConns:
                if conn[preGidIndex] == "NetStim":
                    prePopLabel = (
                        conn[preLabelIndex] if preLabelIndex in conn else "NetStim"
                    )
                else:
                    preCell = next(
                        (cell for cell in cellsPre if cell["gid"] == conn[preGidIndex]),
                        None,
                    )
                    ...
                    if feature in ["weight", "strength"]:
                        if removeWeightNorm:
                            try:
                                ...
                                weightMatrix[
                                    popIndsPre[prePopLabel],
                                    popIndsPost[cell["tags"]["pop"]],
                                ] += (
                        ...
                        delayMatrix[
                            popIndsPre[prePopLabel], popIndsPost[cell["tags"]["pop"]]
                        ] += conn[delayIndex]
                    countMatrix[
                        popIndsPre[prePopLabel], popIndsPost[cell["tags"]["pop"]]
                    ] += 1
    ...
    # Calculate matrix if grouped by numeric tag (eg. 'y')
    elif groupBy in sim.net.allCells[0]["tags"] and isinstance(
        sim.net.allCells[0]["tags"][groupBy], Number
    ):
        if not isinstance(groupByIntervalPre, Number) or not isinstance(
            groupByIntervalPost, Number
        ):
        ...
        minValuePre = _roundFigures(
            groupByIntervalPre * np.floor(min(cellValuesPre) / groupByIntervalPre), 3
        )
        maxValuePre = _roundFigures(
            groupByIntervalPre * np.ceil(max(cellValuesPre) / groupByIntervalPre), 3
        )
        ...
        if includePre == includePost:
            ...
            minValuePost = _roundFigures(
                groupByIntervalPost
                * np.floor(min(cellValuesPost) / groupByIntervalPost),
                3,
            )
            maxValuePost = _roundFigures(
                groupByIntervalPost
                * np.ceil(max(cellValuesPost) / groupByIntervalPost),
                3,
            )
        ...
        for groupPre in groupsPre:
            numCellsGroupPre[groupPre] = len(
                [
                    cell
                    for cell in cellsPre
                    if groupPre
                    <= cell["tags"][groupBy]
                    < (groupPre + groupByIntervalPre)
                ]
            )
        ...
        for preGroup in groupsPre:
            for postGroup in groupsPost:
                ...
                if feature == "convergence":
                    maxPostConnMatrix[
                        groupIndsPre[preGroup], groupIndsPost[postGroup]
                    ] = numCellsGroupPost[postGroup]
                if feature == "divergence":
                    maxPreConnMatrix[
                        groupIndsPre[preGroup], groupIndsPost[postGroup]
                    ] = numCellsGroupPre[preGroup]
        ...
        for cell in cellsPost:  # for each postsyn cell
            ...
            for conn in cellConns:
                if conn[preGidIndex] == "NetStim":
                    ...
                    preCell = next(
                        (c for c in cellsPre if c["gid"] == conn[preGidIndex]), None
                    )
                    if preCell:
                        preGroup = _roundFigures(
                            groupByIntervalPre
                            * np.floor(preCell["tags"][groupBy] / groupByIntervalPre),
                            3,
                        )
                    else:
                        preGroup = None
                ...
                postGroup = _roundFigures(
                    groupByIntervalPost
                    * np.floor(cell["tags"][groupBy] / groupByIntervalPost),
                    3,
                )
    ...
    if groupBy != "cell":
        if feature == "weight":
            if logPlot:
                connMatrix = np.log10(
                    weightMatrix / countMatrix
                )  # avg log weight per conn
            else:
                connMatrix = weightMatrix / countMatrix  # avg weight per conn
                connMatrix = np.nan_to_num(
                    connMatrix, nan=0
                )  # if the count is 0 we get NaNs, but the weight is 0
        elif feature == "delay":
            connMatrix = delayMatrix / countMatrix
            connMatrix = np.nan_to_num(
                connMatrix, nan=0
            )  # if the count is 0 we get NaNs, but the delay is 0
...
def _plotConnCalculateFromFile(
    ...
    if tagsFile:
        ...
        tags = {
            int(k): v for k, v in tagsTmp.items()
        }  # find method to load json with int keys?
        del tagsTmp
    if connsFile:
    ...
    # Calculate matrix if grouped by pop
        ...
        for pop in popsPre:
            if pop in netStimPopsPre:
                ...
                numCellsPopPre[pop] = len(
                    [gid for gid in cellGidsPre if tags[gid][popIndex] == pop]
                )
        ...
        if includePre == includePost:
            ...
            for pop in popsPost:
                if pop in netStimPopsPost:
                    ...
                    numCellsPopPost[pop] = len(
                        [gid for gid in cellGidsPost if tags[gid][popIndex] == pop]
                    )
        ...
        for postGid in cellGidsPost:  # for each postsyn cell
            ...
            for conn in cellConns:
                if conn[preGidIndex] == "NetStim":
                    prePopLabel = (
                        conn[preLabelIndex] if preLabelIndex >= 0 else "NetStims"
                    )
                else:
                    preCellGid = next(
                        (gid for gid in cellGidsPre if gid == conn[preGidIndex]), None
                    )
                ...
                if prePopLabel in popIndsPre:
                    if feature in ["weight", "strength"]:
                        weightMatrix[
                            popIndsPre[prePopLabel],
                            popIndsPost[tags[postGid][popIndex]],
                        ] += conn[weightIndex]
                    elif feature == "delay":
                        delayMatrix[
                            popIndsPre[prePopLabel],
                    ...
                    countMatrix[
                        popIndsPre[prePopLabel], popIndsPost[tags[postGid][popIndex]]
                    ] += 1
    ...
    if groupBy != "cell":
        if feature == "weight":
            if logPlot:
                ...
                connMatrix = (
                    weightMatrix / countMatrix
                )  # avg weight per conn (fix to remove divide by zero warning)
                connMatrix = np.nan_to_num(
                    connMatrix, nan=0
                )  # if the count is 0 we get NaNs, but the weight is 0
        elif feature == "delay":
...
def plotConn(
    ...
    if graphType == "matrix":
        ...
        if groupBy == "cell":
            ...
            for ipop, pop in enumerate(popsPre):
                plt.plot(
                    np.array([0, len(popsPost)]) - 0.5,
                    np.array([ipop, ipop]) - 0.5,
                    "-",
                    c=(0.7, 0.7, 0.7),
                )
            ...
            for igroup, group in enumerate(groupsPre):
                plt.plot(
                    np.array([0, len(groupsPre)]) - 0.5,
                    np.array([igroup, igroup]) - 0.5,
                    "-",
                    c=(0.7, 0.7, 0.7),
                )
    ...
    # stacked bar graph
        ...
        if groupBy == "pop":
            ...
            print(
                '  Error: plotConn graphType="bar" with groupBy="cell" not implemented'
            )
            return None
...
def plot2Dnet(
    ...
    if tagsFile:
        ...
        missing = []
        popIndex = (
            tagsFormat.index("pop") if "pop" in tagsFormat else missing.append("pop")
        )
        ...
        if tags:
            ...
            # pop and cell colors
            popColorsTmp = {
                popLabel: colorList[ipop % len(colorList)]
        ...
        popLabels = [
            pop for pop in sim.net.allPops if pop in selectedPops
    ...
    if showConns and not tagsFile:
        for postCell in cells:
            for con in postCell["conns"]:  # plot connections between cells
                if (
                    not isinstance(con["preGid"], basestring)
                    and con["preGid"] in cellGids
                ):
                    posXpre, posYpre = next(
                        (
                            (cell["tags"]["x"], cell["tags"][ycoord])
                            for cell in cells
                            if cell["gid"] == con["preGid"]
                        ),
                        None,
                    )
...
def plot2Dfiring(
    ...
    def animate(
        ...
        if len(out) == 2:
            ...
            posX = np.array(
                [cells[gid]["tags"]["x"] for gid in spkids]
            )  # get all x positions
            posY = np.array(
                [cells[gid]["tags"][ycoord] for gid in spkids]
            )  # get all y positions
    ...
    if not iv:  # plot using Python instead of interviews
            ...
            # numSyns
                ...
                for cellPost in cellsPost:
                    ...
                    for secLabel, sec in cellSecs.items():
                        ...
                        for conn in conns:
                            nsyns[int(round(conn["loc"] * nseg)) - 1] += 1
                        cvals.extend(nsyns)
        ...
        if cvals is not None and len(cvals) > 0:
            sm = plt.cm.ScalarMappable(
                cmap=cmap, norm=plt.Normalize(vmin=np.min(cvals), vmax=np.max(cvals))
            )
    ...
    if tags and conns:
        ...
        for postGid in cellsPostGids:
            preGidsAll = [
                ...
                if isinstance(conn[preGidIndex], Number)
                and conn[preGidIndex] in cellsPreGids + cellsPrePreGids
            ]
            preGids = [gid for gid in preGidsAll if gid in cellsPreGids]
            for preGid in preGids:
                prePreGids = [
                    conn[preGidIndex]
                    for conn in conns[preGid]
                    if conn[preGidIndex] in cellsPrePreGids
                ]
                totCon += 1
        ...
        if sim.cfg.compactConnFormat:
            if "preGid" in sim.cfg.compactConnFormat:
                preGidIndex = sim.cfg.compactConnFormat.index(
                    "preGid"
                )  # using compact conn format (list)
            else:
