        ...
        for seg in sec["hObj"]:
            ...
            if gridX and gridY:  # 2D
                ...
                if x1 == x2 or y1 == y2:
                    ...
                    sigma = (
                        sigma_x1_y1 * abs(x2 - x) * abs(y2 - y)
                        + sigma_x2_y1 * abs(x - x1) * abs(y2 - y)
                        + sigma_x1_y2 * abs(x2 - x) * abs(y - y1)
                        + sigma_x2_y2 * abs(x - x1) * abs(y - y1)
                    ) / (abs(x2 - x1) * abs(y2 - y1))
                ...
                if y1 == y2:
                    ...
                    sigma = (sigma_y1 * abs(y2 - y) + sigma_y2 * abs(y - y1)) / abs(
                        y2 - y1
                    )
...
def subcellularConn(self, allCellTags, allPopTags):
    ...
    for subConnParamTemp in list(
        self.params.subConnParams.values()
        ...
        if preCellsTags and postCellsTags:
            ...
            for postCellGid in postCellsTags:  # for each postsyn cell
                if postCellGid in self.gid2lid:
                        ...
                        for conn in allConns:
                            if not conn["synMech"].startswith("__grouped__"):
                                ...
                                if conn["synMech"] in subConnParam["groupSynMechs"]:
                                    for synMech in [
                                        s
                                        for s in subConnParam["groupSynMechs"]
                                        if s != conn["synMech"]
                                    ]:
                                        connGroup = next(
                                            (
                    ...
                    conns = sorted(
                        conns, key=lambda v: v["sec"] + str(v["loc"]) + str(v["preGid"])
                    )
                    ...
                    # 2D map and 1D map (radial)
                    ...
                    ) and subConnParam["density"]["type"] in ["2Dmap", "1Dmap"]:

                        gridY = subConnParam["density"]["gridY"]
                        gridSigma = subConnParam["density"]["gridValues"]
                        somaX, somaY, _ = self._posFromLoc(
                            postCell.secs["soma"]["hObj"], 0.5
                        )  # get cell pos move method to Cell!
                        if (
                            "fixedSomaY" in subConnParam["density"]
                        ):  # is fixed cell soma y, adjust y grid accordingly
                        ...
                        if subConnParam["density"]["type"] == "2Dmap":  # 2D
                            gridX = [
                                x - somaX for x in subConnParam["density"]["gridX"]
                        ...
                        scaleNumSyn = (
                            float(len(conns)) / float(totSyn) if totSyn > 0 else 0.0
                        )
                        diffList = []
                        ...
                        if totSynRescale < len(conns):
                            ...
                            diffList = sorted(
                                diffList, key=lambda l: l[0], reverse=True
                            )
                    ...
                    for i, (conn, newSec, newLoc) in enumerate(
                        zip(conns, newSecs, newLocs)
                    ):
                        ...
                        if "weightNorm" in postCell.secs[conn["sec"]] and isinstance(
                            ...
                            newWeightNorm = (
                                postCell.secs[newSec]["weightNorm"][
                                    int(round(newLoc * newNseg)) - 1
                                ]
                                if "weightNorm" in postCell.secs[newSec]
                                else 1.0
                            )
                            conn["weight"] = (
                                conn["weight"] / oldWeightNorm * newWeightNorm
                            )
                        ...
                        if (
                            ...
                            connGroup = connsGroup[
                                connGroupLabel
                            ]  # get grouped conn from previously stored dict
                            ...
                            if newWeightNorm:
                                connGroup["weight"] = (
                                    connGroup["weight"] / oldWeightNorm * newWeightNorm
                                )
