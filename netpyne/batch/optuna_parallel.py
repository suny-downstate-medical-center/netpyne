...
def optunaOptim(self, pc):
    ...
    def objective(trial, args):
        ...
        if type == "mpi_bulletin":
            ...
            # ----------------------------------------------------------------------
            pc.submit(
                runJob, nrnCommand, script, cfgSavePath, netParamsSavePath, jobPath
            )
            print("-" * 80)
            ...
            if mpiCommand == "":
                ...
                command = "%s -n %d %s %s simConfig=%s netParams=%s " % (
                    mpiCommand,
                    numproc,
                    nrnCommand,
                    script,
    ...
    args["direction"] = (
        self.optimCfg["direction"] if "direction" in self.optimCfg else "minimize"
    )
    ...
    sleep(
        rank
    )  # each process wiats a different time to avoid saturating sqlite database
    study = optuna.create_study(
        study_name=self.batchLabel,
        storage="sqlite:///%s/%s_storage.db" % (self.saveFolder, self.batchLabel),
        load_if_exists=True,
    ...
    try:
        study.optimize(
            ...
            n_trials=args["maxiters"],
            timeout=args["maxtime"],
        )
    except Exception as e:
        print(e)
