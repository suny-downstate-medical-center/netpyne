...
def gatherData(gatherLFP=True):
    ...
    simDataVecs = ["spkt", "spkid", "stims", "dipole"] + list(
    ...
    if sim.nhosts > 1:  # only gather if >1 nodes
        netPopsCellGids = {
            popLabel: list(pop.cellGids) for popLabel, pop in sim.net.pops.items()
        }
        ...
        if getattr(sim.cfg, "gatherOnlySimData", False):
            ...
            if sim.rank == 0:  # simData
                ...
                sim.allSimData = Dict()
                for k in list(
                    gather[0]["simData"].keys()
                ):  # initialize all keys of allSimData dict
                    if gatherLFP and k == "LFP":
                        sim.allSimData[k] = np.zeros(
                            (gather[0]["simData"]["LFP"].shape)
                        )
                    elif sim.cfg.recordDipoles and k == "dipole":
                        for dk in sim.cfg.recordDipoles:
                            sim.allSimData[k][dk] = np.zeros(
                                len(gather[0]["simData"]["dipole"][dk])
                            )
                    else:
                        sim.allSimData[k] = {}
                ...
                for node in gather:  # concatenate data from each node
                    for key, val in node[
                        "simData"
                    ].items():  # update simData dics of dics of h.Vector
                        if key in simDataVecs:  # simData dicts that contain Vectors
                            ...
                            sim.allSimData[key].update(
                                val
                            )  # update simData dicts which are not Vectors
                ...
                if len(sim.allSimData["spkt"]) > 0:
                    ...
                    sim.allSimData["spkt"], sim.allSimData["spkid"] = list(
                        sim.allSimData["spkt"]
        ...
        for popLabel, pop in sim.net.pops.items():
            sim.net.allPops[
                popLabel
            ] = pop.__getstate__()  # can't use dict comprehension for OrderedDict
        sim.allSimData = Dict()
        ...
        for key, val in sim.simData.items():  # update simData dics of dics of h.Vector
            if (
                key in simDataVecs + singleNodeVecs
            ):  # simData dicts that contain Vectors
                if isinstance(val, dict):
    ...
    if sim.rank == 0:
        ...
        if sim.cfg.createPyStruct:
            if sim.cfg.compactConnFormat:
                preGidIndex = (
                    sim.cfg.compactConnFormat.index("preGid")
        ...
        if sim.totalSpikes > 0:
            sim.firingRate = (
                float(sim.totalSpikes) / sim.numCells / sim.cfg.duration * 1e3
        ...
        if sim.numCells > 0:
            ...
            sim.synsPerCell = sim.totalSynapses / float(
                sim.numCells
            )  # Calculate the number of connections per cell
        else:
            sim.connsPerCell = 0
        ...
        if sim.totalSynapses != sim.totalConnections:
            print(
                (
                    "  Synaptic contacts: %i (%0.2f per cell)"
        ...
        if "runTime" in sim.timingData:
            ...
            print(
                (
                    "  Simulated time: %0.1f s; %i workers"
            ...
            if sim.cfg.printPopAvgRates and not sim.cfg.gatherOnlySimData:
                ...
                trange = (
                    sim.cfg.printPopAvgRates
                    if isinstance(sim.cfg.printPopAvgRates, list)
                    else None
                )
...
def gatherDataFromFiles(gatherLFP=True, saveFolder=None, simLabel=None, sim=None):
    ...
    if sim.rank == 0:
        ...
        simLabels = [
            f.replace("_node_0.pkl", "")
            for f in os.listdir(nodeDataDir)
            if f.endswith("_node_0.pkl")
        ]
        ...
        for simLabel in simLabels:
            ...
            fileList = sorted(
                [
                    f
                    for f in os.listdir(nodeDataDir)
                    if (f.startswith(simLabel + "_node") and f.endswith(".pkl"))
                ]
            )
            fileType = "pkl"
            if not fileList:
                fileList = sorted(
                    [
                        f
                        for f in os.listdir(nodeDataDir)
                        if (f.startswith(simLabel + "_node") and f.endswith(".json"))
                    ]
                )
                fileType = "json"
                ...
                if fileType == "pkl":
                    ...
                    with open(os.path.join(nodeDataDir, file), "rb") as openFile:
                                    ...
                                    for key2, value2 in value.items():
                                        if isinstance(value2, dict):
                                            ...
                                            allSimData[key][key2] = np.add(
                                                allSimData[key][key2], value2.as_numpy()
                                            )
                                        else:
                                            allSimData[key].update({key2: list(value2)})
                                else:
                                    allSimData[key] = list(allSimData[key]) + list(
                                        value
                                    )
                        ...
                        if file == fileList[0]:
                            ...
                            allPopsCellGids = {
                                popLabel: [] for popLabel in nodePopsCellGids
                            }
                        else:
...
def _gatherAllCellTags():
    ...
    data = [
        {cell.gid: cell.tags for cell in sim.net.cells}
...
def _aggregateDipoles():
    ...
    for k in sim.cfg.recordDipoles:
        sim.simData["dipole"][k] = sim.h.Vector(
