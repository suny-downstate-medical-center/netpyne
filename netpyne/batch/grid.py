...
def gridSearch(self, pc):
    ...
    if self.method == "grid":
        ...
        if ungroupedParams:
            labelList, valuesList = zip(
                *[(p["label"], p["values"]) for p in self.params if p["group"] == False]
            )
        ...
        if groupedParams:
            labelListGroup, valuesListGroup = zip(
                *[(p["label"], p["values"]) for p in self.params if p["group"] == True]
            )
    ...
    for iCombG, pCombG in zip(indexCombGroups, valueCombGroups):
        for iCombNG, pCombNG in zip(indexCombinations, valueCombinations):
            ...
            elif self.runCfg.get("skipCustom", None) and glob.glob(
                jobName + self.runCfg["skipCustom"]
            ):
                print(
                    "Skipping job %s since %s file already exists..."
                    % (jobName, self.runCfg["skipCustom"])
                )
            else:
...
%s
                    ...
                    proc = Popen(
                        ["qsub", batchfile], stderr=PIPE, stdout=PIPE
...
wait
                    ...
                    # subprocess.call
                    proc = Popen(
                        ["sbatch", batchfile], stdin=PIPE, stdout=PIPE
                    )  # Open a pipe to the qsub command.
                    (output, input) = (proc.stdin, proc.stdout)
                ...
                # eg. usage: python batch.py
                    ...
                    command = (
                        "%s -n %d nrniv -python -mpi %s simConfig=%s netParams=%s"
                        % (mpiCommand, cores, script, cfgSavePath, netParamsSavePath)
                    )
