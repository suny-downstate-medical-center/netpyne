...
def load(
    filename, fileformat=None, cell=None, use_axon=True, xshift=0, yshift=0, zshift=0
):
    """
        ...
        for (j, path) in enumerate(seg_paths):
            ...
            if cvals is not None:
                if isinstance(cvals[i], numbers.Number):
                    ...
                    try:
                        col = cmap(
                            int((cvals[i] - clim[0]) * 255 / (clim[1] - clim[0]))
                        )
                    except:
                        col = cmap(0)
                else:
...
def shapeplot_animate(
    v, lines, nframes=None, tscale="linear", clim=[-80, 50], cmap=cm.YlOrBr_r
):
    ...
    if tscale == "linear":
        ...
        def animate(i):
            ...
            for i_seg in range(v.shape[1]):
                lines[i_seg].set_color(
                    cmap(int((v[i_t, i_seg] - clim[0]) * 255 / (clim[1] - clim[0])))
                )
            return []
...
def mark_locations(h, section, locs, markspec="or", **kwargs):
    ...
    # plot markers
    (line,) = plt.plot(
        xyz_marks[:, 0], xyz_marks[:, 1], xyz_marks[:, 2], markspec, **kwargs
    )
    return line
