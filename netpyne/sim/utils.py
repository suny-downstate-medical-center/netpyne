...
def getCellsList(include, returnGids=False):
    ...
    for condition in include:
        ...
        elif isinstance(condition, tuple) or isinstance(
            condition, list
        ):  # subset of a pop with relative indices
            cellsPop = [
                gid for gid, tags in allCellTags.items() if tags["pop"] == condition[0]
            ]
            ...
            if hasattr(sim, "rank"):
                if sim.rank == 0:
                    if mode == "start":
                        ...
                        sim.timingData[processName] = (
                            time() - sim.timingData[processName]
                        )
            else:
                if mode == "start":
                    sim.timingData[processName] = time()
...
def replaceFuncObj(obj):
    ...
    if type(obj) == list:
        ...
        for key, val in obj.items():
            ...
            if "func_name" in dir(
                val
            ):  # hasattr(val,'func_name'):  # avoid hasattr() since it creates key in Dicts()
...
def _mat2dict(obj):
    ...
    if isinstance(obj, dict):
        ...
        out = []
        for item in obj:
            if isinstance(item, spio.matlab.mio5_params.mat_struct) or isinstance(
                item, np.ndarray
            ):
...
def clearAll():
    ...
    if hasattr(sim, "net"):
        sim.clearObj(
            [
                cell.__dict__ if hasattr(cell, "__dict__") else cell
