...
def fix_sec_nseg(secs, dL):
    ...
    for secName in secs:
        secs[secName]["geom"]["nseg"] = 1 + 2 * int(
            secs[secName]["geom"]["L"] / (2 * dL)
        )
...
class SONATAImporter:
    ...
    def importNet(self, configFile, replaceAxon=True, setdLNseg=True, swapSomaXY=True):
        ...
        if "mechanisms_dir" in self.network_config["components"]:
            modFolder = (
                self.subs(self.network_config["components"]["mechanisms_dir"])
                + "/modfiles"
            )
    ...
    def createSimulationConfig(self):
        ...
        sim.cfg.pt3dRelativeToCellLocation = (
            False  # Make cell 3d points relative to the cell x,y,z location
        )
        ...
        sim.net.params.defaultThreshold = self.simulation_config["run"][
            "spike_threshold"
        ]
        ...
        if "node_sets_file" in self.simulation_config:
            ...
            sim.cfg.node_sets = load_json(
                self.subs(
                    self.rootFolder + "/" + self.simulation_config["node_sets_file"]
                ).replace("$BASE_DIR", "")
            )
    ...
    def createPops(self):
        ...
        for sonata_pop in self.cell_info:
            ...
            for type in self.cell_info[sonata_pop]["type_numbers"]:
                ...
                model_template = (
                    info["model_template"] if "model_template" in info else "- None -"
                )
                ...
                print(
                    " - Adding population: %s which has model info: %s" % (pop_id, info)
                )
                ...
                popTags["cellType"] = (
                    info["model_name"] if "model_name" in info else pop_id
                )
                ...
                if model_type == "biophysical":
                    ...
                    if info["model_template"].startswith("nml"):
                        ...
                        cellRule = self.setCellRuleDynamicParamsFromJson(
                            cell_dynamic_params, cellRule
                        )
    ...
    def createCells(self):
        for sonata_pop in self.cell_info:
            ...
            for sonata_group in sonata_groups:
                cellLocs[sonata_group] = self.cell_info[sonata_pop][sonata_group][
                    "locations"
                ]
            ...
            self.cell_info[sonata_pop][
                "gid_from_id"
            ] = {}  # keep track of gid as func of cell id
            ...
            for icell in _distributeCells(numCells)[sim.rank]:
                ...
                if model_type == "biophysical":
                    cellTags["x"] = cellLocs[node_group_id][icell][
                        "x"
                    ]  # set x location (um)
                    cellTags["y"] = (
                        -1 * cellLocs[node_group_id][icell]["y"]
                    )  # set y location (um) (reversed since netpyne assumes depth)
                    cellTags["z"] = cellLocs[node_group_id][icell][
                        "z"
                    ]  # set z location (um)
                    cellTags["xnorm"] = (
                        cellTags["x"] / sim.net.params.sizeX
                    )  # set x location (um)
                    cellTags["ynorm"] = (
                        cellTags["y"] / sim.net.params.sizeY
                    )  # set y location (um)
                    cellTags["znorm"] = (
                        cellTags["z"] / sim.net.params.sizeZ
                    )  # set z location (um)
                    ...
                    if "spkTimes" in pop.tags:  # if VecStim, copy spike times to params
                        ...
                        if isinstance(pop.tags["spkTimes"][0], list):
                            try:
                                cellTags["params"]["spkTimes"] = pop.tags["spkTimes"][
                                    icell
                                ]  # 2D list
                            except:
                                pass
                        else:
                            cellTags["params"]["spkTimes"] = pop.tags[
                                "spkTimes"
                            ]  # 1D list (same for all)

                sim.net.cells.append(
                    pop.cellModelClass(gid, cellTags)
                )  # instantiate Cell object
                print(
                    (
                        "Cell %d/%d (gid=%d) of pop %s, on node %d, "
                        % (icell, numCells, gid, pop_id, sim.rank)
                    )
                )
        ...
        for conn in self.conn_info:
            ...
            for type in self.edges_info[conn]:
                ...
                if syn_label not in sim.net.params.synMechParams:
                    dynamics_params_file = (
                        self.subs(
                            self.network_config["components"]["synaptic_models_dir"]
                        )
                        + "/"
            ...
            for i in range(len(self.conn_info[conn]["pre_id"])):
                ...
                if post_gid in sim.net.gid2lid:
                    ...
                    # synMech
                    connParams["synMech"] = self.edges_info[conn][type][
                        "dynamics_params"
                    ].split(".")[0]
        ...
        for sec, sec_type in section_lists:
            for prop_name, prop_obj in nml_params[sec_type].items():
                if prop_obj.element_tag() == "resistivity":
                    ...
                    prop_obj.element_tag() == "channelDensity"
                    and prop_obj.ion_channel == "pas"
                ):
                    cellRule["secs"][sec]["mechs"]["pas"] = {
                        "g": prop_obj.cond_density,
                        "e": prop_obj.erev,
                    }

                elif (
                    prop_obj.element_tag() == "channelDensity"
                    or prop_obj.element_tag() == "channelDensityNernst"
                ):
                    ...
                    cellRule["secs"][sec]["mechs"][prop_obj.id] = {
                        "gamma": prop_obj.gamma,
                        "decay": prop_obj.decay,
                    }
    ...
    def setCellRuleDynamicParamsFromNeuroml_old(self, cell, cellRule):
        ...
        seg_grps_vs_nrn_sections = {
            segGroup: [sec for sec in cellRule["secs"] if sec.startswith(segGroup)]
            for segGroup in segGroupKeys
        }
        seg_grps_vs_nrn_sections["all"] = list(cellRule["secs"])
        inhomogeneous_parameters = {
            segGroup: [] for segGroup in segGroupKeys
        }  # how to fill in this from swc file?
        ...
        for (
            cm
        ) in cell.biophysical_properties.membrane_properties.channel_density_v_shifts:
        ...
        for (
            cm
        ) in cell.biophysical_properties.membrane_properties.channel_density_nernsts:
            group = "all" if not cm.segment_groups else cm.segment_groups
            for section_name in seg_grps_vs_nrn_sections[group]:
                gmax = pynml.convert_to_units(cm.cond_density, "S_per_cm2")
                if cm.ion_channel == "pas":
                ...
                if vp.parameter == "condDensity":
                    ...
                    for section_name in seg_grps_vs_nrn_sections[grp]:
                        ...
                        gmax_start = pynml.convert_to_units(
                            "%s S_per_m2" % eval(expr), "S_per_cm2"
                        )
                        p = path_end
        ...
        for vi in cell.biophysical_properties.membrane_properties.init_memb_potentials:
            ...
            for section_name in seg_grps_vs_nrn_sections[group]:
                cellRule["secs"][section_name]["vinit"] = pynml.convert_to_units(
                    vi.value, "mV"
                )

        # remove default vinit if vi empty so the global h.v_init is used
        if (
            len(cell.biophysical_properties.membrane_properties.init_memb_potentials)
            == 0
        ):
            group = "all"
            for section_name in seg_grps_vs_nrn_sections[group]:
        ...
        for sc in cell.biophysical_properties.membrane_properties.specific_capacitances:
            ...
            for section_name in seg_grps_vs_nrn_sections[group]:
                cellRule["secs"][section_name]["geom"]["cm"] = pynml.convert_to_units(
                    sc.value, "uF_per_cm2"
                )
        ...
        if hasattr(
            ...
            for (
                ...
                for section_name in seg_grps_vs_nrn_sections[group]:
                    cellRule["secs"][section_name]["geom"][
                        "Ra"
        ...
        if hasattr(cell, "concentrationModel"):
            ...
            for param in cell.concentratrionModel:
                if param not in excludeConcentrationModel:
                    concentrationModelParams[cell.concentratrionModel.id][
                        param
                    ] = getattr(cell.concentratrionModel, param)
        ...
        if hasattr(cell.biophysical_properties.intracellular_properties, "species"):
            ...
            for specie in cell.biophysical_properties.intracellular_properties.species:
                ...
                for section_name in seg_grps_vs_nrn_sections[group]:
                    cellRule["secs"][section_name]["ions"][specie.ion][
                        "o"
                    ] = pynml.convert_to_units(specie.initial_ext_concentration, "mM")
                    cellRule["secs"][section_name]["ions"][specie.ion][
                        "i"
    ...
    def setCellRuleDynamicParamsFromJson(self, cell_dynamic_params, cellRule):
        ...
        for p in genome:
            ...
            for sec in sections:
                if p["mechanism"] != "":
                    cellRule["secs"][sec]["mechs"][p["mechanism"]] = {
                        p["name"].split("_")[0]: p["value"]
                    }

        # Set reversal potentials
        for erev in conditions["erev"]:
            sections = [
                s for s in cellRule["secs"] if s.split("_")[0] == erev["section"]
            ]
            for sec in sections:
                for eion in erev:
        ...
        for node in g:
            ...
            if node._c_classid == "GROUP":
                ...
                if g._v_name == self.current_edge:
                    ...
                    print(
                        "  Found edge %s -> %s"
                        % (self.current_pre_node, self.current_post_node)
                    )
                    self.conn_info[self.current_edge][
                        "pre_node"
    ...
    def parse_dataset(self, d):
        ...
        if self.current_node_group:
            for i in range(0, d.shape[0]):
                ...
                if (
                    not i
                    in self.cell_info[self.current_node][self.current_node_group][
                        "locations"
                    ]
                ):
                    self.cell_info[self.current_node][self.current_node_group][
