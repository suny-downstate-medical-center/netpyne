...
def iplotRaster(
    ...
    **kwargs
    ...
    df = pd.concat(
        [df.drop("tags", axis=1), pd.DataFrame.from_records(df["tags"].tolist())],
        axis=1,
    )
    ...
    popLabels = [
        pop for pop in sim.net.allPops if pop in df["pop"].unique()
    ...
    if len(cellGids) > 0:
        gidColors = {
            cell["gid"]: popColorDict[cell["tags"]["pop"]] for cell in cells
        }  # dict with color for each gid
        try:
            sel, spkts, spkgids = getSpktSpkid(
                cellGids=[] if include == ["allCells"] else cellGids,
                timeRange=timeRange,
            )
        except:
            import sys
    ...
    for netStimLabel in netStimLabels:
        ...
        if len(netStimSpks) > 0:
            ...
            ns = pd.DataFrame(
                list(zip(spktsNew, spkindsNew)), columns=["spkt", "spkind"]
            )
    ...
    if len(sel) > maxSpikes:
        print(
            ("  Showing only the first %i out of %i spikes" % (maxSpikes, len(sel)))
        )  # Limit num of spikes
    ...
    popNumCells = (
        [float(gidPops.count(pop)) for pop in popLabels]
        if numCellSpks
        else [0] * len(popLabels)
    )
    totalSpikes = len(sel)
    ...
    if popRates:
        ...
        if numNetStims:
            ...
            avgRates["NetStims"] = (
                len([spkid for spkid in sel["spkind"].iloc[numCellSpks:]])
                / numNetStims
                / tsecs
            )
    ...
    if syncLines:
        for spkt in sel["spkt"].tolist():
            fig.line(
                (spkt, spkt), (0, len(cells) + numNetStims), color="red", line_width=2
            )
        print(syncMeasure())
        ...
        t.text = "cells=%i  syns/cell=%0.1f  rate=%0.1f Hz" % (
            numCells,
            connsPerCell,
            firingRate,
        )
    ...
    if spikeHist:
        histo = np.histogram(
            sel["spkt"].tolist(),
            bins=np.arange(timeRange[0], timeRange[1], spikeHistBin),
        )
    ...
    if spikeHist:
        ...
        fig.extra_y_ranges = {
            "spikeHist": Range1d(start=min(histoCount), end=max(histoCount))
        }
        fig.add_layout(
            LinearAxis(y_range_name="spikeHist", axis_label="Spike count"), "right"
        )
    ...
    **kwargs
    ...
    for key in dpl.keys():
        ...
        if sim.cfg.hnn_params["dipole_smooth_win"] > 0:
            dpl[key] = hammfilt(
                dpl[key], sim.cfg.hnn_params["dipole_smooth_win"] / sim.cfg.dt
            )
...
def iplotDipoleSpectrogram(
    ...
    **kwargs
    ...
    def baseline_renormalize():
        ...
        # L2 dipole offset can be roughly baseline shifted over the entire range of t
        dpl = {
            "L2": np.array(sim.simData["dipole"]["L2"]),
...
def iplotSpikeHist(
    ...
    **kwargs
    ...
    for iplot, subset in enumerate(include):
            ...
            try:
                spkinds, spkts = list(
                    zip(
                        *[
                            (spkgid, spkt)
                            for spkgid, spkt in zip(
                                sim.allSimData["spkid"], sim.allSimData["spkt"]
                            )
        ...
        if yaxis == "rate":
            histoCount = (
                histoCount * (1000.0 / binSize) / (len(cellGids) + numNetStims)
        ...
        if isinstance(subset, list):
            ...
            color = (
                popColorDict[subset]
                if subset in popColorDict
                else colors[iplot % len(colors)]
            )
...
def iplotRatePSD(
    ...
    **kwargs
    ...
    for iplot, subset in enumerate(include):
        ...
        s = fig.line(
            freqs[freqs < maxFreq], signal[freqs < maxFreq], line_width=2.0, color=color
        )
        if overlay:
...
def iplotTraces(
    ...
    **kwargs
    ...
    if include is None:  # if none, record from whatever was recorded
        if (
            "plotTraces" in sim.cfg.analysis
            and "include" in sim.cfg.analysis["plotTraces"]
        ):
            ...
            for itrace, trace in enumerate(tracesList):
                if "cell_{}".format(gid) in sim.allSimData[trace]:
                    ...
                    if isinstance(fullTrace, dict):
                        data = [
                            fullTrace[key][
                                int(timeRange[0] / recordStep) : int(
                                    timeRange[1] / recordStep
                                )
                            ]
                    ...
                    if overlay:
                        fig.line(
                            ...
                            data,
                            line_width=2,
                            line_color=colors[itrace],
                            legend_label=trace,
                        )
                        hover = HoverTool(
                            tooltips=[("Time", "@x"), ("Value", "@y")], mode="vline"
                        )
                        fig.add_tools(hover)
                        fig.legend.click_policy = "hide"
                    else:
    ...
    for figLabel, figObj in figs.items():
        ...
        if saveFig:
            if isinstance(saveFig, str):
                ...
                filename = (
                    sim.cfg.filename + "_itraces" + figLabel + overlay_text + ".html"
                )
            file = open(filename, "w")
            file.write(html)
            file.close()
    ...
    **kwargs
    ...
    if "PSD" in plots:
        ...
        for i, elec in enumerate(electrodes):
            ...
            hover = HoverTool(
                tooltips=[("Frequency", "@x"), ("Power", "@y")], mode="vline"
            )
            p.add_tools(hover)
            ...
            if transformMethod == "morlet":
                ...
                Fs = int(1000.0 / sim.cfg.recordStep)
                morletSpec = MorletSpec(
                    lfpPlot, Fs, freqmin=minFreq, freqmax=maxFreq, freqstep=stepFreq
                )
        ...
        html = file_html(
            plot_layout, CDN, title="LFP Power Spectral Density", theme=theme
        )
    ...
    if "spectrogram" in plots:
        ...
        if transformMethod == "morlet":
            ...
            for i, elec in enumerate(electrodes):
                ...
                mapper = linear_cmap(
                    field_name="dB/Hz", palette=Viridis256, low=vmin, high=vmax
                )
                color_bar = ColorBar(
    ...
    **kwargs
    ...
    # stacked bar graph
        ...
        if groupBy == "pop":
            ...
            print(
                '  Error: plotConn graphType="bar" with groupBy="cell" not yet implemented'
            )
            return None
    ...
    plot_layout = layout([fig], sizing_mode="stretch_both")
    html = file_html(
        plot_layout, CDN, title="Connection " + feature + " matrix", theme=theme
    )
...
def iplot2Dnet(
    ...
    **kwargs
    ...
    print(
        "Plotting interactive 2D representation of network cell locations and connections..."
    )
    ...
    if tagsFile:
        ...
        cellColors = [
            "#%02x%02x%02x"
            % tuple([int(col * 255) for col in popColors[cell["tags"]["pop"]]])
            for cell in cells
        ]
    ...
    fig.scatter(
        posX, posY, radius=radius, fill_color=cellColors, line_color=None
    )  # plot cell soma positions
...
# -------------------------------------------------------------------------------------------------------------------
@exception
def iplotRxDConcentration(
    speciesLabel, regionLabel, plane="xy", saveFig=None, showFig=True, **kwargs
):
    """
    ...
    **kwargs
    ...
    for stat in stats:
            ...
            if stat in statDataIn:
                ...
                if "stims" in sim.allSimData:
                    for netStimLabel in netStimLabels:
                        ...
                        if len(netStimSpks) > 0:
                            ...
                            spktsNew = netStimSpks
                            spkindsNew = [
                                lastInd + 1 + i for i in range(len(netStimSpks))
                            ]
                            spkts.extend(spktsNew)
                            spkinds.extend(spkindsNew)
                            numNetStims += 1
                ...
                if stat == "rate":
                    ...
                    if includeRate0:
                        ...
                        rates = (
                            [spkinds.count(gid) * toRate for gid in set(spkinds)]
                            if len(spkinds) > 0
                            else [0]
                        )  # cellGids] #set(spkinds)]
                ...
                # Inter-spike interval (ISI) coefficient of variation (CV) stats
                    ...
                    spkmat = [
                        [spkt for spkind, spkt in zip(spkinds, spkts) if spkind == gid]
                        for gid in set(spkinds)
                    ]
                    isimat = [
                        [t - s for s, t in zip(spks, spks[1:])]
                        for spks in spkmat
                        if len(spks) > 10
                    ]
                    isicv = [
                        np.std(x) / np.mean(x) if len(x) > 0 else 0 for x in isimat
                    ]  # if len(x)>0]
        ...
        if graphType == "boxplot":
            ...
            # stems
            fig.segment(
                labels, upper, labels, q3, line_color=line_color, line_width=line_width
            )
            fig.segment(
                labels, lower, labels, q1, line_color=line_color, line_width=line_width
            )
    ...
    if not spks1:
        ...
        for netStimPop in netStimPops:
            if "stims" in sim.allSimData:
                ...
                if len(cellStims) > 0:
                    spktsNew = [
                        spkt for cellStim in cellStims for spkt in cellStim[netStimPop]
                    ]
                    spkts.extend(spktsNew)
