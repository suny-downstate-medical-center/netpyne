...
class Cell(object):
    ...
    def recordStimSpikes(self):
        ...
        for conn in self.conns:
            if conn["preGid"] == "NetStim":
                ...
                sim.simData["stims"]["cell_" + str(self.gid)].update(
                    {conn["preLabel"]: stimSpikeVecs}
                )
    ...
    def calculateCorrectBorderDist(self):
        ...
        for icoord, coord in enumerate(coords):
            ...
            if coord + "borders" in sim.net.params.correctBorder:
                borders = [
                    b * size for b in sim.net.params.correctBorder[coord + "borders"]
                ]
    ...
    def _shapeStim(
        ...
        if stimshape == "gaussian":
            pulse = exp(
                -2 * (2 * x / width - 1) ** 2
            )  # Offset by 2 standard deviations from start
            pulse = pulse / max(pulse)
        elif stimshape == "square":
            pulse = zeros(shape(x))
            pulse[
                int(npts / 2) : int(npts / 2) + int(width / timeres)
        ...
        fulltime = (
            r_[0:allpts] * timeres + start
        ) * 1e3  # Create time vector and convert to ms
        ...
        fulloutput = hstack(
            (0, fulloutput, 0)
        )  # Set weight to zero at either end of the stimulus period
        events = hstack((0, events, 0))  # Ditto
        stimvecs = deepcopy(
            [fulltime, fulloutput, events]
    ...
    def addNetStim(self, params, stimContainer=None):
        ...
        if not stimContainer:
            ...
            if sim.cfg.verbose:
                print(
                    (
                        "  Created %s NetStim for cell gid=%d"
                        % (params["source"], self.gid)
                    )
                )
            ...
            if isinstance(params["rate"], basestring):
                if params["rate"] == "variable":
                    try:
                        ...
                        print(
                            "Error: tried to create variable rate NetStim but NSLOC mechanism not available"
                        )
                else:
                    print(
                        "Error: Unknown stimulation rate type: %s" % (h.params["rate"])
                    )
            else:
                netstim = h.NetStim()
                netstim.interval = (
    ...
    def recordTraces(self):
        ...
        for key, params in sim.cfg.recordTraces.items():
            ...
            if conditionsMet:
                try:
                    ...
                    if "loc" in params and params["sec"] in self.secs:
                        if "mech" in params:  # eg. soma(0.5).hh._ref_gna
                            ptr = getattr(
                                getattr(
                                    self.secs[params["sec"]]["hObj"](params["loc"]),
                                    params["mech"],
                                ),
                                "_ref_" + params["var"],
                            )
                            ...
                            if len(synMechList) > 0:
                                if "index" in params:
                                    if params["index"] < len(synMechList):
                                        ...
                                        ptr = getattr(
                                            synMech["hObj"], "_ref_" + params["var"]
                                        )
                                else:
                                    synMech = synMechList[
                                        0
                                    ]  # 0th one which would have been returned by next()
                                    ptr = getattr(
                                        synMech["hObj"], "_ref_" + params["var"]
                                    )
                        elif (
                            "stim" in params
                        ):  # e.g. sim.net.cells[0].stims[0]['hObj'].i
                        ...
                        "synMech" in params
                    ):  # special case where want to record from multiple synMechs
                        if "sec" in params:
                            sec = self.secs[params["sec"]]
                            synMechs = [
                                synMech
                            ...
                            ptr = [
                                getattr(synMech["hObj"], "_ref_" + params["var"])
                                for synMech in synMechs
                            ]
                            secLocs = [
                                params.sec + str(synMech["loc"]) for synMech in synMechs
                            ]
                        else:
                            ptr = []
                            secLocs = []
                            ...
                            if "mech" in params:
                                ...
                                for conn_idx, conn in enumerate(self.conns):
                                    if params["mech"] in conn.keys():
                                        ...
                                        secLocs.extend(
                                            [params["sec"] + "_conn_" + str(conn_idx)]
                                        )
                            else:
                                print(
                    ...
                    if ptr:  # if pointer has been created, then setup recording
                        if isinstance(ptr, list):
                            ...
                            for ptrItem, secLoc in zip(ptr, secLocs):
                                ...
                                if (
                                    hasattr(sim.cfg, "use_local_dt")
                                    and sim.cfg.use_local_dt
                                ):
                                    self.secs[params["sec"]]["hObj"].push()
                                    if (
                                        hasattr(sim.cfg, "recordStep")
                ...
                if sim.cfg.verbose:
                    print(
                        "  Conditions preclude recording ", key, " from cell ", self.gid
                    )
        # else:
