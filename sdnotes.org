...
3) in /utilities/ python run-dev-server.py

4) checkout and pull metadata branch in netpyne repo

** Extend GUI
- In browser open console + select "No cache" -- so changes in js take effect after reloading if run-dev-server.py running
