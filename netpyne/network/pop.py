...
class Pop(object):
    ...
    def __init__(self, label, tags):
        self.tags = (
            tags  # list of tags/attributes of population (eg. numCells, cellModel,...)
        )
        self.tags["pop"] = label
    ...
    def createCellsFixedNum(self):
        ...
        cells = []
        self.rand.Random123(
            self.tags["numCells"], sim.net.lastGid, sim.cfg.seeds["loc"]
        )
        self.rand.uniform(0, 1)
        vec = h.Vector(self.tags["numCells"] * 3)
        vec.setrand(self.rand)
        randLocs = np.array(vec).reshape(
            self.tags["numCells"], 3
        )  # create random x,y,z locations

        if sim.net.params.shape == "cylinder":
            # Use the x,z random vales
            rho = randLocs[
                :, 0
            ]  # use x rand value as the radius rho in the interval [0, 1)
            phi = (
                2 * pi * randLocs[:, 2]
            )  # use z rand value as the angle phi in the interval [0, 2*pi)
            ...
            phi = (
                2 * pi * randLocs[:, 1]
            )  # use y rand value as the angle phi in the interval [0, 2*pi)
            costheta = (
                2 * randLocs[:, 2]
        ...
        for icoord, coord in enumerate(["x", "y", "z"]):
            if (
                coord + "Range" in self.tags
            ):  # if user provided absolute range, convert to normalized
            ...
            if (
                coord + "normRange" in self.tags
            ):  # if normalized range, rescale random locations
        ...
        for i in self._distributeCells(numCells)[sim.rank]:
            ...
            if "spkTimes" in self.tags:  # if VecStim, copy spike times to params
                if isinstance(self.tags["spkTimes"][0], list):
                    try:
                        cellTags["params"]["spkTimes"] = self.tags["spkTimes"][
                            i
                        ]  # 2D list
                    except:
                        pass
                else:
                    cellTags["params"]["spkTimes"] = self.tags[
                        "spkTimes"
            ...
            if "dynamicRates" in self.tags:  # if NetStim, copy rates array to params
                if (
                    "rates" in self.tags["dynamicRates"]
                    and "times" in self.tags["dynamicRates"]
                ):
            ...
            if sim.cfg.verbose:
                print(
                    (
                        ...
                        % (
                            i,
                            sim.net.params.scale * self.tags["numCells"] - 1,
                            gid,
                            self.tags["pop"],
                            sim.rank,
                        )
    ...
    def createCellsDensity(self):
        ...
        for coord in ["x", "y", "z"]:
            if (
                ...
                self.tags[coord + "normRange"] = [
                    point / getattr(sim.net.params, "size" + coord.upper())
                    for point in self.tags[coord + "Range"]
                ]
        ...
        if isinstance(
            self.tags["density"], basestring
        ):  # check if density is given as a function
            if shape == "cuboid":  # only available for cuboids
                ...
                maxDensity = max(
                    list(map(densityFunc, (np.arange(minRange, maxRange, interval))))
                )  # max cell density
                maxCells = (
                    volume * maxDensity
                )  # max number of cells based on max value of density func

                self.rand.Random123(
                    int(maxDensity), sim.net.lastGid, sim.cfg.seeds["loc"]
                )
                ...
                funcLocs = [
                    ...
                    if i in np.array(makethiscell.nonzero()[0], dtype="int")
                ...
                print(
                    "Error: Density functions are only implemented for cuboid shaped networks"
                )
                exit(0)
        else:  # NO ynorm-dep
            self.tags["numCells"] = int(
                self.tags["density"] * volume
            )  # = density (cells/mm^3) * volume (mm^3)
        ...
        if sim.cfg.verbose and not funcLocs:
            print(
                "Volume=%.4f, density=%.2f, numCells=%.0f"
            ...
            if sim.cfg.verbose:
                print(
                    (
                        ...
                        % (i, self.tags["numCells"] - 1, gid, i, sim.rank)
                    )
                )
    ...
    def createCellsGrid(self):
        ...
        rangeLocs = [
            [0, getattr(sim.net.params, "size" + coord)] for coord in ["X", "Y", "Z"]
        ]
        for icoord, coord in enumerate(["x", "y", "z"]):
            # constrain to range set by user
            if (
                coord + "normRange" in self.tags
            ):  # if normalized range, convert to normalized
            ...
            if (
                coord + "Range" in self.tags
            ):  # if user provided absolute range, calculate range
                self.tags[coord + "normRange"] = [
                    float(point) / getattr(sim.net.params, "size" + coord.upper())
                    for point in self.tags[coord + "Range"]
                ]
                rangeLocs[icoord] = [
                    self.tags[coord + "Range"][0],
                    self.tags[coord + "Range"][1],
                ]
        ...
        if isinstance(gridSpacing, list):
            for x in np.arange(rangeLocs[0][0], rangeLocs[0][1] + 1, gridSpacing[0]):
                for y in np.arange(
                    rangeLocs[1][0], rangeLocs[1][1] + 1, gridSpacing[1]
                ):
                    for z in np.arange(
                        rangeLocs[2][0], rangeLocs[2][1] + 1, gridSpacing[2]
                    ):
            ...
            for x in np.arange(rangeLocs[0][0], rangeLocs[0][1] + 1, gridSpacing):
                for y in np.arange(rangeLocs[1][0], rangeLocs[1][1] + 1, gridSpacing):
                    for z in np.arange(
                        rangeLocs[2][0], rangeLocs[2][1] + 1, gridSpacing
                    ):
                        gridLocs.append((x, y, z))
        ...
        for i in self._distributeCells(numCells)[sim.rank]:
            ...
            cellTags["pop"] = self.tags["pop"]
            cellTags["xnorm"] = (
                gridLocs[i][0] / sim.net.params.sizeX
            )  # set x location (um)
            cellTags["ynorm"] = (
                gridLocs[i][1] / sim.net.params.sizeY
            )  # set y location (um)
            cellTags["znorm"] = (
                gridLocs[i][2] / sim.net.params.sizeZ
            )  # set z location (um)
            ...
            if sim.cfg.verbose:
                print(
                    (
                        "Cell %d/%d (gid=%d) of pop %s, on node %d, "
                        % (i, numCells, gid, self.tags["pop"], sim.rank)
                    )
                )
    ...
    def __getstate__(self):
        ...
        odict = sim.replaceFuncObj(
            odict
        )  # replace h objects with None so can be pickled
