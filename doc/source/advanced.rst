...
The ``importCellParams(self, label, conds, fileName, cellName, cellArgs={}, importSynMechs=False)`` method takes as arguments the cell rule where to store the imported cell properties, the name of the file where the cell is defined (either .py or .hoc files), and the name of the cell template (hoc) or class (python). Optionally, a set of arguments can be passed to the cell template/class (eg. ``{'type': 'RS'}``); and the synaptic mechanisms parameters can also be read from the file and stored in ``synMechParams``.
