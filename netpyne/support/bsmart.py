...
def timefreq(x, fs=200):
    ...
    F = np.arange(
        minfreq, maxfreq + minfreq, minfreq
    )  # Create frequencies evenly spaced from 0:minfreq:maxfreq
...
def armorf(x, ntrls, npts, p):
    ...
    for i in range(ntrls):
        ...
        ap = (
            ap
            + x[:, i * npts + 1 : (i + 1) * npts]
            * x[:, i * npts + 1 : (i + 1) * npts].H
        )
        bp = (
            bp
            + x[:, i * npts : (i + 1) * npts - 1]
            * x[:, i * npts : (i + 1) * npts - 1].H
        )
    ...
    for m in range(p):
        ...
        for k in range(ntrls):
            ...
            for i in range(m + 2):
                ...
                ebp = ebp + np.matrix(b[:, :, m + 1 - i]) * np.matrix(
                    x[:, k1 - 1 : k2 - 1]
                )
            pf = pf + efp * efp.H
            pb = pb + ebp * ebp.H
    ...
    for j in range(p):
        coeff = np.concatenate(
            (coeff, inv(np.matrix(a[:, :, 0])) * np.matrix(a[:, :, j + 1])), 1
        )
