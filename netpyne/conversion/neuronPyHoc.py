...
def importCellParams(fileName, labels, values, key=None):
    ...
    if fileName.endswith(".py"):
        try:
            ...
            if (
                filePath not in sys.path
            ):  # add to path if not there (need to import module)
            ...
            moduleName = fileNameOnly.split(".py")[
                0
            ]  # remove .py to obtain module name
...
def importCell(fileName, cellName, cellArgs=None, cellInstance=False):
    ...
    origGlob[
        "v_init"
    ] = -65  # add by hand since won't be set unless load h.load_file('stdrun')
...
def importCellsFromNet(
    netParams, fileName, labelList, condsList, cellNamesList, importSynMechs
):
    """
...
def getCellParams(cell, varList={}, origGlob={}):
    ...
    dirCellHnames = {}
    for dirCellName in [
        d for d in dirCell if not d.startswith("__")
        ...
        if isinstance(dirCellObject, list):
            for i, dirCellObjectItem in enumerate(dirCellObject):
                try:
                    if (
                        dirCellObjectItem.hname() not in dirCellHnames
                    ):  # give preference to dict entries
                        dirCellHnames.update(
                            {dirCellObjectItem.hname(): dirCellName + "_" + str(i)}
                        )
                except:
                    pass
    ...
    for sec in secs:
        dirCellSecNames.update(
            {
                hname: name
                for hname, name in dirCellHnames.items()
                if hname == sec.hname()
            }
        )
    ...
    for sec in secs:
        ...
        for mech in dir(sec(0.5)):
            if (
                h.ismembrane(mech) and mech not in ignoreMechs
            ):  # check if membrane mechanism
                if not mech.endswith("_ion"):  # exclude ions
                    ...
                    for varName in varNames:
                        if varName not in ignoreVars:
                            try:
                                varVals = [
                                    seg.__getattribute__(mech).__getattribute__(varName)
                                    for seg in sec
                                ]
                        ...
                        if varName not in ignoreVars:
                            try:
                                if varNameSplit in [
                                    ...
                                    varVals = [
                                        seg.__getattribute__(ionName + varNameSplit)
                                        for seg in sec
                                    ]
                                else:  # var name before ion name (eg. 'ena')
                                    varVals = [
                                        seg.__getattribute__(varNameSplit + ionName)
                                        for seg in sec
                                    ]
                                if len(set(varVals)) == 1:
                                    varVals = varVals[0]
        ...
        for seg in sec:
            for ipoint, point in enumerate(seg.point_processes()):
                ...
                if any(
                    ...
                    if not any(
                        [
                            _equal_dicts(synMech, synMech2, ignore_keys=["label"])
                    ...
                    for varName in varNames:
                        try:
                            pointps[pointpName][varName] = point.__getattribute__(
                                varName
                            )
    ...
    if (
        "v_init" in globs
    ):  # set v_init for each section (allows for cells with differnet vinits)
