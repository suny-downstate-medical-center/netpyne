    ...
    for condition in include:
        if condition == "all":  # all cells + Netstims
            ...
            if condition in allNetStimLabels:
                ...
                cellGids.extend(
                    [c["gid"] for c in allCells if c["tags"]["pop"] == condition]
                )
        ...
        # when load from json gets converted to list (added as exception)
            ...
            if isinstance(condition[1], list):
                cellGids.extend(
                    [gid for i, gid in enumerate(cellsPop) if i in condition[1]]
                )
            elif isinstance(condition[1], int):
                cellGids.extend(
                    [gid for i, gid in enumerate(cellsPop) if i == condition[1]]
                )
            ...
            for subcond in condition:
                if isinstance(subcond, int):  # cell gid
                    ...
                    if subcond in allNetStimLabels:
                        ...
                        cellGids.extend(
                            [c["gid"] for c in allCells if c["tags"]["pop"] == subcond]
                        )
    ...
    if tagsFormat or "format" in allCells:
        ...
        for condition in include:
            if condition in ["all", "allCells"]:  # all cells
                ...
                cellGids.extend(
                    [gid for gid, c in allCells.items() if c[popIndex] == condition]
                )

            elif isinstance(condition, tuple):  # subset of a pop with relative indices
                cellsPop = [
                    gid for gid, c in allCells.items() if c[popIndex] == condition[0]
                ]
                if isinstance(condition[1], list):
                    cellGids.extend(
                        [gid for i, gid in enumerate(cellsPop) if i in condition[1]]
    ...
    # using dict with keys
        ...
        for condition in include:
            if condition in ["all", "allCells"]:  # all cells
                ...
                cellGids.extend(
                    [gid for gid, c in allCells.items() if c["pop"] == condition]
                )

            elif isinstance(condition, tuple):  # subset of a pop with relative indices
                cellsPop = [
                    gid for gid, c in allCells.items() if c["pop"] == condition[0]
                ]
                if isinstance(condition[1], list):
                    cellGids.extend(
                        [gid for i, gid in enumerate(cellsPop) if i in condition[1]]
...
def getSpktSpkid(cellGids=[], timeRange=None, sim=None):
    ...
    df = pd.DataFrame(
        pandaslib.to_object_array(
            [sim.allSimData["spkt"], sim.allSimData["spkid"]]
        ).transpose(),
        columns=["spkt", "spkid"],
    )
    ...
    return (
        ...
        sel["spkt"].tolist(),
        sel["spkid"].tolist(),
    )  # will want to return sel as well for further sorting
...
def checkAvailablePlots(requireCfg=False):
    ...
    if (
        hasattr(sim, "net")
        and hasattr(sim.net, "allCells")
        and len(sim.net.allCells) > 0
    ):
        avail["plotConn"] = True
    ...
    if (
        hasattr(sim, "allSimData")
        and "LFP" in sim.allSimData
        and len(sim.allSimData["LFP"]) > 0
    ):
