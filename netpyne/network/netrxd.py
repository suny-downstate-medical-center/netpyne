...
def addRxD(self, nthreads=None):
    ...
    if sim.cfg.createNEURONObj:
        ...
        if "multicompartmentReactions" in rxdParams:
            self._addReactions(
                rxdParams["multicompartmentReactions"], multicompartment=True
            )
...
def _addRegions(self, params):
    ...
    for label, param in params.items():
        ...
        if isinstance(geometry, dict):
            try:
                if "args" in geometry:
                    geometry["hObj"] = getattr(rxd, param["geometry"]["class"])(
                        **param["geometry"]["args"]
                    )
...
def _addExtracellularRegion(self, label, param):
    ...
    for arg in requiredArgs:
        if arg not in param:
            print(
                "  Error creating Extracellular object %s: %s parameter was missing"
                % (label, arg)
            )
...
def _addSpecies(self, params):
    ...
    for label, param in params.items():
        ...
        if "regions" not in param:
            print(
                '  Error creating Species %s: "regions" parameter was missing' % (label)
            )
            continue
        ...
        if isinstance(param["initial"], basestring):  # string-based func
            funcStr = self._replaceRxDStr(
                param["initial"], constants=True, regions=True, species=False
            )

            # create final function dynamically from string
            importStr = " from neuron import crxd as rxd \n from netpyne import sim"
            afterDefStr = 'sim.net.rxd["species"]["%s"]["initialFunc"] = initial' % (
                label
            )
...
def _addStates(self, params):
    ...
    for label, param in params.items():
        ...
        if "regions" not in param:
            print(
                '  Error creating State %s: "regions" parameter was missing' % (label)
            )
            continue
        if not isinstance(param["regions"], list):
            param["regions"] = [param["regions"]]
        try:
            ...
            print(
                "  Error creating State %s: could not find regions %s"
                % (label, param["regions"])
            )
        ...
        # call rxd method to create Region
        if nrnRegions:
            self.rxd["states"][label]["hObj"] = rxd.State(
                regions=nrnRegions, initial=initial, name=name
            )
        else:
...
def _addParameters(self, params):
    ...
    for label, param in params.items():
        ...
        try:
            nrnRegions = [
                self.rxd["regions"][region]["hObj"] for region in param["regions"]
            ]
        except:
            print(
                "  Error creating State %s: could not find regions %s"
        ...
        if isinstance(param["value"], basestring):
            funcStr = self._replaceRxDStr(
                param["value"], constants=True, regions=True, species=True
            )
...
def _replaceRxDStr(
    ...
    if regions and "regions" in self.rxd:
        for regionLabel in self.rxd["regions"]:
            mapping[regionLabel] = 'sim.net.rxd["regions"]["%s"]["hObj"]' % (
                regionLabel
            )
    ...
    if species and "species" in self.rxd:
        for speciesLabel in self.rxd["species"]:
            mapping[speciesLabel] = 'sim.net.rxd["species"]["%s"]["hObj"]' % (
                speciesLabel
            )
    ...
    if parameters and "parameters" in self.rxd:
        for paramLabel in self.rxd["parameters"]:
            mapping[paramLabel] = 'sim.net.rxd["parameters"]["%s"]["hObj"]' % (
                paramLabel
            )
