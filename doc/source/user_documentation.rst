...
* **recordLFPSourcePops** - Optional source-population selection for each electrode in ``recordLFP``. Provide a parallel list whose entries are a population label, a list of labels, or ``None`` for all populations, e.g. ``[['E'], ['I'], None]``. Repeated electrode locations may use different source selections. When omitted or ``None``, ``LFP`` retains its historical all-compartmental-populations behavior. This option filters only the summed ``LFP`` signal; ``saveLFPPops`` and ``saveLFPCells`` retain their historical unfiltered contribution semantics. (default: None).
