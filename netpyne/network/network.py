...
class Network(object):
    ...
    def createPops(self):
        ...
        for (
            popLabel,
            popParam,
        ) in self.params.popParams.items():  # for each set of population paramseters
    ...
    def createCells(self):
        ...
        if sim.rank == 0:
            print(
                (
                    "\nCreating network of %i cell populations on %i hosts..."
                    % (len(self.pops), sim.nhosts)
                )
            )
        ...
        for ipop in list(
            self.pops.values()
        ):  # For each pop instantiate the network cells (objects of class 'Cell')
            ...
            if sim.rank == 0 and sim.cfg.verbose:
                print(
                    (
                        "Instantiated %d cells of population %s"
                        % (len(newCells), ipop.tags["pop"])
                    )
                )
        ...
        if sim.rank == 0 and sim.cfg.timing:
            print(
                ("  Done; cell creation time = %0.2f s." % sim.timingData["createTime"])
            )
