...
def asd(
    ...
    **kwargs
    ...
    if randseed is not None:
        ...
        if verbose >= 3:
            print(
                "ASD: Launching with random seed is %i; sample: %f"
                % (randseed, nr.random())
            )
    ...
    if sinitial is None:
        ...
        stepsizes = np.concatenate(
            (stepsizes, stepsizes)
        )  # need to duplicate since two for each parameter
    else:
    ...
    for x in xPop:
        if sum(np.isnan(x)):
            errormsg = (
                "ASD: At least one value in the vector of starting points is NaN:\n%s"
                % x
            )
            raise Exception(errormsg)
    if label is None:
    ...
    # Initialize history
    abserrorhistoryPop = [
        dcp(np.zeros(stalliters)) for i in range(popsize)
    ]  # Store previous error changes
    relerrorhistoryPop = [
        dcp(np.zeros(stalliters)) for i in range(popsize)
    ]  # Store previous error changes
    fvalsPop = [
        dcp(np.zeros(maxiters + 1)) for i in range(popsize)
    ]  # Store all objective function values
    allstepsPop = [
        dcp(np.zeros((maxiters + 1, nparams))) for i in range(popsize)
    ...
    while True:
        ...
        for icand, (x, fval, fvalnew, probabilities, stepsizes) in enumerate(
            ...
            if verbose == 1:
                print(
                    offset
                    + label
                    + "Iteration %i; elapsed %0.1f s; objective: %0.3e"
                    % (count, time() - start, fval)
                )  # For more verbose, use other print statement below
            if verbose >= 4:
                print(
                    "\n\n Count=%i \n x=%s \n probabilities=%s \n stepsizes=%s"
                    % (count, x, probabilities, stepsizes)
                )

            if fvalnew == maxFitness:
                print(
                    "Note: rerunning candidate %i since it did not complete in previous iteration ...\n"
                    % (icand)
                )
                xnew = dcp(
                    x
                ...
                for r in range(maxrangeiters):  # Try to find parameters within range
                    choice = np.flatnonzero(cumprobs > nr.random())[
                        0
                    ]  # Choose a parameter and upper/lower at random
                    par = np.mod(choice, nparams)  # Which parameter was chosen
                    pm = np.floor((choice) / nparams)  # Plus or minus
                    newval = (
                        x[par] + ((-1) ** pm) * stepsizes[choice]
                ...
                if (
                    not inrange
                ):  # Treat it as a failure if a value in range can't be found
            ...
            (
                ...
                stepsizesPop[icand],
            ) = (x, fval, probabilities, stepsizes)
        ...
        for icand, (
            ...
            if fvalnew == -1:
                ...
                try:
                    if abs(fvalnew) < eps and abs(fval) < eps:
                        ...
                        ratio = (
                            1.0 / eps
                        )  # Only the denominator is zero: reset to the maximum ratio
                    else:
                        ratio = fval / float(
                            fvalnew
                        )  # The normal situation: calculate the real ratio
                except:
                    ratio = 1.0
                ...
                if fvalnew < fvalold:  # New parameter set is better than previous one
                    ...
                    stepsizes[choice] = (
                        stepsizes[choice] * sinc
                    )  # Increase size of step for next time
                    ...
                    stepsizes[choice] = (
                        stepsizes[choice] / sdec
                    )  # Decrease size of step for next time
                    flag = "--"  # Marks no change
                    if np.isnan(fvalnew):
                        if verbose >= 1:
            ...
            if verbose >= 2:
                print(
                    ...
                    % (
                        (icand, count, time() - start, flag)
                        + sigfig([fvalorig, fvalold, fvalnew, fvalnew - fvalold])
                    )
                )
        ...
        if (time() - start) > maxtime:
            exitreason = "Time limit reached (%s > %s)" % sigfig(
                [(time() - start), maxtime]
            )
            break
    ...
    output["details"]["fvals"] = [
        fvals[: count + 1] for fvals in fvalsPop
...
def runASDJob(script, cfgSavePath, netParamsSavePath, simDataPath):
    ...
    command = "nrniv %s simConfig=%s netParams=%s" % (
        script,
        cfgSavePath,
        netParamsSavePath,
    )
    ...
    with open(simDataPath + ".run", "w") as outf, open(
        simDataPath + ".err", "w"
    ) as errf:
        pid = Popen(
            command.split(" "), stdout=outf, stderr=errf, preexec_fn=os.setsid
        ).pid
...
def asdOptim(self, pc):
    ...
    def evaluator(candidates, args):
        ...
        for candidate_index, candidate in enumerate(candidates):
            ...
            if type == "mpi_bulletin":
                ...
                # ----------------------------------------------------------------------
                command = (
                    "%s -n %d nrniv -python -mpi %s simConfig=%s netParams=%s "
                    % (mpiCommand, numproc, script, cfgSavePath, netParamsSavePath)
                )
                ...
                if type == "mpi_direct":
                    with open(jobPath + ".run", "a+") as outf, open(
                        jobPath + ".err", "w"
                    ) as errf:
                        pids.append(
                            Popen(
                    ...
                    with open(jobPath + ".jobid", "w") as outf, open(
                        jobPath + ".err", "w"
                    ) as errf:
                        pids.append(
                            Popen(
        ...
        # print outfilestem
        print(
            "Waiting for jobs from generation %d/%d ..." % (ngen, args.get("maxiters"))
        )
        # print "PID's: %r" %(pids)
        ...
        while jobs_completed < total_jobs:
            ...
            if num_iters >= args.get("maxiter_wait", 5000):
                ...
                for canditade_index in unfinished:
                    fitness[
                        canditade_index
                    ] = maxFitness  # rerun those that didn't complete;
                    jobs_completed += 1
                    try:
    ...
    if len(self.params[0]["values"]) > 2 and isinstance(
        self.params[0]["values"][2], list
    ):
    ...
    # if no 3rd value, calculate random values
        ...
        for p in range(popsize):
            x0.append(
                [np.random.uniform(x["values"][0], x["values"][1]) for x in self.params]
            )
    ...
    kwargs["args"][
        "cfg"
    ] = self.cfg  # include here args/params to pass to evaluator function
    kwargs["args"]["paramLabels"] = [x["label"] for x in self.params]
    kwargs["args"]["netParamsSavePath"] = (
        self.saveFolder + "/" + self.batchLabel + "_netParams.py"
    )
    kwargs["args"]["maxiters"] = (
        self.optimCfg["maxiters"] if "maxiters" in self.optimCfg else 1000
    )
