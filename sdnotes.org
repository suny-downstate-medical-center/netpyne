...
TODO:
- first trial PMd input
- with 120 sec, some periods no input
- maxPMdrate variables
- reduce minNoise so no sync spikes every 10 sec! or increase noisefraction
- increase noise fraction so spikes not synced
- check PMD->ER5 connections -- seems similar acitivty for both targets
