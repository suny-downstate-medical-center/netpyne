...
def saveData(include=None, filename=None):
    ...
    if (
        sim.rank == 0
        and not getattr(sim.net, "allCells", None)
        and not getattr(sim, "allSimData", None)
    ):
        needGather = True
    else:
        needGather = False
    ...
    if sim.rank == 0:
        ...
        if isinstance(sim.cfg.backupCfgFile, list) and len(sim.cfg.backupCfgFile) == 2:
            simName = (
                sim.cfg.simLabel
                if sim.cfg.simLabel
                else os.path.basename(sim.cfg.filename)
            )
        ...
        if dataSave:
            ...
            if hasattr(sim.cfg, "saveFolder") and hasattr(sim.cfg, "simLabel"):
                filePath = os.path.join(
                    sim.cfg.saveFolder, sim.cfg.simLabel + "_data" + timestampStr
                )
            else:
            ...
            if sim.cfg.saveMat:
                ...
                savemat(
                    filePath + ".mat", utils.tupleToList(utils.replaceNoneObj(dataSave))
            ...
            if sim.cfg.saveDat:
                ...
                for ref in list(traces.keys()):
                    for cellid in list(sim.allSimData[ref].keys()):
                        ...
                        print(
                            (
                                "Saving %i points of data on: %s:%s to %s"
...
def compactConnFormat():
    ...
    if type(sim.cfg.compactConnFormat) is not list:
        if (
            len(sim.net.params.stimTargetParams) > 0
        ):  # if have stims, then require preLabel field
    ...
    for key, val in simData.items():  # update simData dics of dics of h.Vector
        if key in simDataVecs:  # simData dicts that contain Vectors
            if isinstance(val, dict):
                ...
                saveSimData[key] = list(saveSimData[key]) + list(
                    val
                )  # udpate simData dicts which are Vectors
    ...
    if dataSave:
        ...
        if sim.rank == 0:
            if sim.cfg.timing:
                ...
                print(
                    (
                        "  Done; saving time = %0.2f s."
                        % sim.timingData["saveInNodeTime"]
                    )
                )
            if sim.cfg.timing and sim.cfg.saveTiming:
                import pickle
