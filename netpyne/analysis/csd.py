...
def getCSD(
    ...
    if LFP_input_data is None and LFP_input_file is None:
        ...
        try:
            ...
            raise Exception(
                "No LFP input data, input file, or existing simulation. Cannot calculate CSD."
            )
        ...
        if "LFP" in sim_data_categories:
            ...
            raise Exception(
                "NO LFP DATA!! Need to re-run simulation with cfg.recordLFP enabled."
            )
    ...
    # load sim data from a JSON file
        ...
        for i in data.keys():
            ...
            if sampr is None:
                csd_data[i]["sampr"] = 1.0 / (
                    (data[i]["simConfig"]["recordStep"]) / 1000.0
                )
            ...
            if spacing_um is None:
                csd_data[i]["spacing_um"] = (
                    data[i]["simConfig"]["recordLFP"][1][1]
                    - data[i]["simConfig"]["recordLFP"][0][1]
                )
...
def plotCSD(
    ...
    if CSD_data is None:
        ...
        lfp_data, CSD_data, sampr, spacing_um, dt = getCSD(
            sampr=sampr, spacing_um=spacing_um, dt=dt, getAllData=True
        )
        ...
        if "CSD" in sim_data_categories:
            ...
            # get LFP data
            LFP_data = np.array(sim.allSimData["LFP"])[
                int(timeRange[0] / sim.cfg.recordStep) : int(
                    timeRange[1] / sim.cfg.recordStep
                ),
                :,
            ]
    ...
    if overlay is "CSD_raw" or overlay is "CSD_bandpassed" or overlay is "LFP":
        ...
        if overlay == "CSD_raw":
            ...
            for chan in range(nrow):
                ...
                subaxs[chan].plot(
                    X, CSD_data_noBandpass[chan, :], color="red", linewidth=0.4
                )

        elif overlay == "CSD_bandpassed":
            axs[0].set_title(
                "CSD with Bandpassed CSD time series overlay", fontsize=fontSize
            )
            for chan in range(nrow):
                subaxs.append(plt.Subplot(fig, gs_inner[chan], frameon=False))
                fig.add_subplot(subaxs[chan])
    ...
    if hlines:
        for i in range(len(sim.cfg.recordLFP)):
            axs[0].hlines(
                sim.cfg.recordLFP[i][1], xmin, xmax, colors="black", linestyles="dashed"
            )
    ...
    if layer_lines:
        ...
        for n in range(len(layerKeys)):
            if n == 0:
                ...
                axs[0].text(
                    xmax + 5,
                    (layer_bounds[layerKeys[n]] + layer_bounds[layerKeys[n - 1]]) / 2,
                    layerKeys[n],
                    color="black",
                    fontsize=fontSize,
                )
    ...
    if type(stim_start_time) is int or type(stim_start_time) is float:
        axs[0].vlines(
            stim_start_time, ymin, ymax, colors="red", linewidth=1, linestyles="dashed"
        )
