...
def exportNeuroML2(
    ...
    for np_pop_id in net.pops:
        ...
        if (
            "cellModel" in np_pop.tags
            and not np_pop.tags["cellModel"] == "NetStim"
            and not cell_id in cells_added
        ):
                ...
                for np_sec_name in list(cell_param_set["secs"].keys()):
                    ...
                    mp.specific_capacitances.append(
                        neuroml.SpecificCapacitance(
                            value="%s uF_per_cm2" % cm, segment_groups=nml_seg_group.id
                        )
                    )
                    ...
                    if len(mp.init_memb_potentials) == 0:
                        mp.init_memb_potentials.append(
                            neuroml.InitMembPotential(value="%s mV" % vinit)
                        )

                    if len(mp.spike_threshes) == 0:
                        mp.spike_threshes.append(
                            neuroml.SpikeThresh(
                                value="%s mV" % sim.net.params.defaultThreshold
                            )
                        )
...
try:
    ...
    class NetPyNEBuilder(DefaultNetworkHandler):
        ...
        def handle_network(self, network_id, notes, temperature=None):
            if temperature:
                self.simConfig.hParams["celsius"] = pynml.convert_to_units(
                    temperature, "degC"
                )
                print(
        ...
        def handle_population(
            self, population_id, component, size, component_obj, properties={}
        ):

            if self.verbose:
                print(
                    "A population: %s with %i of %s (%s)"
                    % (population_id, size, component, component_obj)
                )
            ...
            if isinstance(component_obj, Cell):
                ...
                self.pop_ids_vs_use_segment_groups_for_neuron[
                    population_id
                ...
                if not use_segment_groups_for_neuron:
                    ...
                    self.pop_ids_vs_cumulative_lengths[
                        population_id
                    ] = cumulative_lengths
                ...
                for (
                    ...
                    cell.biophysical_properties.membrane_properties.channel_density_ghk2s
                ):
                ...
                for (
                    ...
                    raise Exception(
                        "<channelDensityNonUniformNernst> not yet supported!"
                    )
        ...
        def _convert_to_nrn_section_location(self, population_id, seg_id, fract_along):
            ...
            if not self.pop_ids_vs_use_segment_groups_for_neuron[population_id]:
                ...
                return (
                    self.pop_ids_vs_seg_ids_vs_segs[population_id][seg_id].name,
                    fract_along,
                )
            else:
                fract_sec = -1
