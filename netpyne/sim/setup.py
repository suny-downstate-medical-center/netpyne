...
def initialize(netParams=None, simConfig=None, net=None):
    ...
    if sim.nhosts > 1:
        sim.cfg.checkErrors = False  # turn of error chceking if using multiple cores

    if (
        hasattr(sim.cfg, "checkErrors") and sim.cfg.checkErrors
            ...
            if netParamsPath.endswith(".json"):
                ...
                try:  # py3
                    loader = importlib.machinery.SourceFileLoader(
                        os.path.basename(netParamsPath).split(".")[0], netParamsPath
                    )
...
def setupRecordLFP():
    ...
    sim.net.recXElectrode = RecXElectrode(
        sim
    )  # create exctracellular recording electrode
...
def setupRecording():
    ...
    # spike recording
    sim.simData.update(
        {name: h.Vector(1e4).resize(0) for name in ["spkt", "spkid"]}
    )  # initialize
    if sim.cfg.recordCellsSpikes == -1:
        ...
        recordGidsSpikes = utils.getCellsList(
            sim.cfg.recordCellsSpikes, returnGids=True
        )
    ...
    if "plotRaster" in sim.cfg.analysis:
        if (
            isinstance(sim.cfg.analysis["plotRaster"], dict)
            and "include" in sim.cfg.analysis["plotRaster"]
        ):
    ...
    if "plotSpikeHist" in sim.cfg.analysis:
        if sim.cfg.analysis["plotSpikeHist"] == True:
            ...
            isinstance(sim.cfg.analysis["plotSpikeHist"], dict)
    ...
    if sim.cfg.recordTraces:
        ...
        if (
            ...
            "iplotTraces" in sim.cfg.analysis
            and "include" in sim.cfg.analysis["iplotTraces"]
        ):
        ...
        if sim.cfg.recordTime and len(sim.simData) > 0:
            try:
                sim.simData[
                    "t"
                ] = h.Vector()  # sim.cfg.duration/sim.cfg.recordStep+1).resize(0)
                if hasattr(sim.cfg, "use_local_dt") and sim.cfg.use_local_dt:
                    ...
                    recordStep = (
                        0.1 if sim.cfg.recordStep == "adaptive" else sim.cfg.recordStep
                    )
