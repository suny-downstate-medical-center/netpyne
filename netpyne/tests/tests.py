...
TEST_TYPE_EXISTS_IN_NESTED_DICT = (
    "Exists in nested dict"  # input param must exist in nested dict
)
...
class TestTypeObj(object):
    ...
    def testExistsInPop(self, val, paramValues, pops):
        try:
            ...
            if isinstance(paramValues[val], dict) and "pop" in paramValues[val]:
                popsSpecified = True
                # print( " LLL **** ")
                if isinstance(pops, dict) and paramValues[val]["pop"] not in list(
                    pop.keys()
                ):
                    errorMessage = (
    ...
    def testIsValidRange(self, val, params):  # TEST_TYPE_IS_VALID_RANGE
        try:
            if val in params:
                ...
                )  # check if lower < upper value in range
        except AssertionError as e:
            e.args += (val,)
            raise
        ...
        try:
            ...
            if isinstance(val, list) and isinstance(valList, list):
                assert all([x in valList for x in val]), (
                    str(val[[x in valList for x in val].index(False)])
                    + " must be in list "
                    + str(valList)
                )
        except AssertionError as e:
        ...
        try:
            ...
            if isinstance(paramDict, dict) and isinstance(valList, list):
                ...
                assert all([x in valList for x in list(paramDict.keys())]), (
                    ...
                    + str(
                        list(paramDict.keys())[
                            [x in valList for x in list(paramDict.keys())].index(False)
                        ]
                    )
                    + "': Valid values are: "
                    + str(valList)
    ...
    def testValidStim(self, paramValues):  # TEST_TYPE_VALID_STIM
        try:
            ...
            if isinstance(paramValues, dict):
                ...
                pointpsValues = []
                if "pointps" in mechVarList() and isinstance(
                    mechVarList()["pointps"], dict
                ):
            ...
            if not stimParamsAllowed:
                ...
                if "noise" in paramValues:
                    if not isinstance(paramValues["noise"], numbers.Real):
                        ...
                        float(paramValues["noise"]) < 0
                        or float(paramValues["noise"]) > 1
                    ):
                        stimValid = False
                        errorMessages.append(
                            "popParams->'noise': Must be a float between 0 and 1 if specified. Value provided is:"
                ...
                if paramValues["cellModel"] != "VecStim" and any(
                    [x in paramValues for x in ["spkTimes", "pulses"]]
                ):
                    stimValid = False
                    errorMessages.append(
                    ...
                    if "geom" in values:
                        if len(values["geom"]) == 0:
                            errorMessage = (
                                "cellParams -> secs ('"
                                + str(key)
                                + "'): Geom parameters must be specified."
                            )
                            geomValid = False
                        if not isinstance(values["geom"], dict):
                            errorMessage = (
                                "cellParams -> secs ('"
                                + str(key)
                                + "'): Geom parameters must be specified as a dict."
                            )
                            geomValid = False
                        ...
                        for key1, values1 in list(values["geom"].items()):
                            ...
                            if PT_3D in values["geom"]:
                                if not isinstance(
                                    values["geom"][PT_3D], list
                                ) and not isinstance(values["geom"][PT_3D], tuple):
                                    errorMessage = (
                                        "cellParams -> secs ('"
                                        + str(key)
                                ...
                                for elem in values["geom"][PT_3D]:
                                    if not isinstance(elem, list) and not isinstance(
                                        elem, tuple
                                    ):
                                        errorMessage = (
                                            "cellParams -> secs ('"
                    ...
                    for key, value in list(paramValues["secs"].items()):
                        if "topol" not in value:
                            ...
                            if not isinstance(value["topol"], dict):
                                ...
                                [
                                    x in list(value["topol"].keys())
                                    for x in VALID_TOPOLOGY_PARAMS
                                ]
                            ):
                                topolValid = False
                                errorMessage = (
                                ...
                                value["topol"]["parentX"], numbers.Real
                            ):
                                topolValid = False
                                errorMessage = (
                                    "cellParams -> secs ('"
                                ...
                                value["topol"]["parentX"] < 0
                                or value["topol"]["parentX"] > 1
                            ):
                                topolValid = False
                                errorMessage = (
                                    "cellParams -> secs ('"
                                ...
                                value["topol"]["childX"] < 0
                                or value["topol"]["childX"] > 1
                            ):
                                topolValid = False
                                errorMessage = (
                                    "cellParams -> secs ('"
                    ...
                    if "mechs" in value:
                        for key1, values1 in list(paramValues["secs"][key].items()):
                            if key1 == "mechs":
                                ...
                                for key2, values2 in list(
                                    paramValues["secs"][key][key1].items()
                                ):
                                    keys_suffixed = []
                                    mechsValidFlag = True
                                    ...
                                    if key2.find("_ion") != -1:
                                        ...
                                        mechs[key2] = [
                                            x.replace(ionName, "") for x in mechs[key2]
                                        ]
                                        keys_suffixed = [
                                            x for x in list(values2.keys())
                                        ]
                                        if not any(
                                            [
                                                x in mechs[key2]
                                                for x in list(values2.keys())
                                            ]
                                        ):
                                        ...
                                        keys_suffixed = [
                                            x + "_" + key2 for x in list(values2.keys())
                                        ]
                                        mechs_unsuffixed = [
                                            x.replace("_" + key2, "")
                                            for x in mechs[key2]
                                        ...
                                        if not all(
                                            [x in mechs[key2] for x in keys_suffixed]
                                        ):
                                            mechsValidFlag = False
                                            errorMessage = (
                                                "cellParams -> secs ('"
                                            ...
                                            [x in keys_suffixed for x in mechs[key2]]
                                        ):
                                            mechsWarningFlag = True
                                            errorMessage = (
                                                "cellParams -> secs ('"
    ...
    def checkSyncMechs(
        self, paramValue, values, dimValues, dimSynMechs, synsPerConn
    ):  # check syncMechs
                ...
                for synMech in synMechs:
                    if not synMech in netParams.synMechParams:
                        errorMessage = (
                            "ConnParams -> synMechs -> "
                            + synMech
                            + ": Synaptic mechanism is not valid."
                        )
                        synMechsValid = False
    ...
    def testValidSynMechs(
        self, parameterName, paramValues, netParams
                ...
                if parameterName == "loc":
                    ...
                    if not all([isinstance(x, numbers.Real) for x in flattenedValues]):
                        errorMessage = (
                            "ConnParams -> "
                            + str(parameterName)
                            + " must be a number with values between 0 and 1."
                        )
                        synMechsValid = False
    ...
    def testValidConnList(self, paramValues, netParams):  # TEST_TYPE_VALID_CONN_LIST
        ...
        try:
            ...
            # print ( " in check 1 synsPerConn 222 dimSynMechs = " + str(dimSynMechs) + " synsPerConn = " + str(synsPerConn))
            errorMessage = self.checkConnList(
                parameterName, values, dimValues, dimSynMechs, synsPerConn
            )
        ...
        try:
            ...
            if switchOnOff != "":
                if not isinstance(switchOnOff, list):
                    errorMessage = (
                        "connList -> shape: SwitchOnOff, if specified, must be a list."
                    )
                    errorMessages.append(errorMessage)
                elif not all([isinstance(x, numbers.Real) for x in switchOnOff]):
                    errorMessage = (
                        "connList -> shape: SwitchOnOff values must be numeric."
                    )
                    errorMessages.append(errorMessage)
                        ...
                        for key1, value1 in list(value.items()):
                            if isinstance(value1, dict) and "spikeGenLoc" in value1:
                                ...
                                if not isinstance(spikeGenLoc, numbers.Real):
                                    errorMessages.append(
                                        "cellParams -> secs -> spikeGenLoc must be a float."
                                    )
                                    return errorMessages
                                elif spikeGenLoc < 0 or spikeGenLoc > 1:
                                    errorMessages.append(
                                        "cellParams -> secs -> spikeGenLoc must be between 0 and 1."
                                    )
                                    return errorMessages
        ...
        try:
            ...
            if simConfig and simConfig.analysis:
                ...
                if not isinstance(simConfig.analysis, dict):
                    errorMessages.append(
                        "SimConfig->'analysis': Must be a dict. Value provided is "
                        + str(simConfig.analysis)
                        + "."
                    )
                    return errorMessages
                # print (" before ")
                ...
                if "plotRaster" in analysis:
                    ...
                    if not isinstance(plotRaster, dict) and not isinstance(
                        plotRaster, bool
                    ):
                        errorMessages.append(
                        ...
                        if "include" in plotRaster and not isinstance(
                            plotRaster["include"], dict
                        ):
                            errorMessages.append(
                        ...
                        if "maxSpikes" in plotRaster and not isinstance(
                            plotRaster["maxSpikes"], int
                        ):
                            errorMessages.append(
                        ...
                        if "labels" in plotRaster:
                            ...
                            if any(
                                [
                                    x not in ["legend", "overlay", "y"]
                                    for x in plotRaster["labels"]
                                ]
                            ):
                                errorMessages.append(
                        ...
                        if "popRates" in plotRaster:
                            ...
                            if not isinstance(
                                plotRaster["popRates"], basestring
                            ) and not isinstance(plotRaster["popRates"], bool):
                                errorMessages.append(
                                ...
                                if (
                                    isinstance(plotRaster["popRates"], basestring)
                                ) and (x not in ["legend", "overlay"]):
                                    errorMessages.append(
                ...
                if "plotSpikeHist" in analysis:
                    ...
                    if not isinstance(plotSpikeHist, dict) and not isinstance(
                        plotSpikeHist, bool
                    ):
                        errorMessages.append(
                        ...
                        if "include" in plotSpikeHist and not isinstance(
                            plotSpikeHist["include"], list
                        ):
                            errorMessages.append(
                        ...
                        if "orderInverse" in plotSpikeHist:
                            ...
                            if not isinstance(
                                plotSpikeHist["orderInverse"], bool
                            ) and not isinstance(plotSpikeHist, dict):
                                errorMessages.append(
                        ...
                        if "graphType" in plotSpikeHist and plotSpikeHist[
                            "graphType"
                        ] not in ["line", "bar"]:
                            errorMessages.append(
                        ...
                        if "yaxis" in plotSpikeHist and plotSpikeHist["yaxis"] not in [
                            "rate",
                            "count",
                        ]:
                            errorMessages.append(
                        ...
                        if "figSize" in plotSpikeHist and not isinstance(
                            plotSpikeHist["figSize"], tuple
                        ):
                            errorMessages.append(
                        ...
                        if "binSize" in plotSpikeHist and not isinstance(
                            plotSpikeHist["binSize"], int
                        ):
                            errorMessages.append(
                ...
                if "plotSpikePSD" in analysis:
                    ...
                    if not isinstance(plotSpikePSD, dict) and not isinstance(
                        plotSpikePSD, bool
                    ):
                        errorMessages.append(
                        ...
                        if "include" in plotSpikePSD and not isinstance(
                            plotSpikePSD["include"], list
                        ):
                            errorMessages.append(
                        ...
                        if "Fs" in plotSpikePSD and not isinstance(
                            plotSpikePSD["Fs"], numbers.Real
                        ):
                            errorMessages.append(
                        ...
                        if "figSize" in plotSpikePSD and not isinstance(
                            plotSpikePSD["figSize"], tuple
                        ):
                            errorMessages.append(
                        ...
                        if "binSize" in plotSpikePSD and not isinstance(
                            plotSpikePSD["binSize"], int
                        ):
                            errorMessages.append(
                ...
                if "plotTraces" in analysis:
                    ...
                    if not isinstance(plotTraces, dict) and not isinstance(
                        plotTraces, bool
                    ):
                        errorMessages.append(
                        ...
                        if "include" in plotTraces and not isinstance(
                            plotTraces["include"], list
                        ):
                            errorMessages.append(
                        ...
                        if "overlay" in plotTraces and not isinstance(
                            plotTraces["overlay"], bool
                        ):
                            errorMessages.append(
                        ...
                        if "binSize" in plotTraces and not isinstance(
                            plotTraces["binSize"], int
                        ):
                            errorMessages.append(
                        ...
                        if "oneFigPer" in plotTraces and plotTraces[
                            "oneFigPer"
                        ] not in ["cell", "trace"]:
                            errorMessages.append(
                        ...
                        if "rerun" in plotTraces and not isinstance(
                            plotTraces["rerun"], bool
                        ):
                            errorMessages.append(
                        ...
                        if "figSize" in plotTraces and not isinstance(
                            plotTraces["figSize"], tuple
                        ):
                            errorMessages.append(
                        ...
                        if "saveFig" in plotTraces and not isinstance(
                            plotTraces["saveFig"], tuple
                        ):
                            errorMessages.append(
                ...
                if "plotShape" in analysis:
                    ...
                    if not isinstance(plotShapes, dict) and not isinstance(
                        plotShapes, bool
                    ):
                        errorMessages.append(
                        ...
                        if "showSyns" in plotShapes and not isinstance(
                            plotShape["showSyns"], bool
                        ):
                            errorMessages.append(
                        ...
                        if "showFig" in plotShapes and not isinstance(
                            plotShape["showFig"], bool
                        ):
                            errorMessages.append(
                ...
                if "plotConn" in analysis:
                    ...
                    if not isinstance(plotConn, dict) and not isinstance(
                        plotConn, bool
                    ):
                        errorMessages.append(
                        ...
                        if "include" in plotConn and not isinstance(
                            plotConn["include"], list
                        ):
                            errorMessages.append(
                        ...
                        if "showFig" in plotConn and not isinstance(
                            plotConn["showFig"], bool
                        ):
                            errorMessages.append(
                ...
                if "plot2DNet" in analysis:
                    ...
                    if not isinstance(plot2DNet, dict) and not isinstance(
                        plot2DNet, bool
                    ):
                        errorMessages.append(
                        ...
                        if "include" in plot2DNet and not isinstance(
                            plot2DNet["include"], list
                        ):
                            errorMessages.append(
                        ...
                        if "showConns" in plot2DNet and not isinstance(
                            plot2DNet["showConns"], bool
                        ):
                            errorMessages.append(
                        ...
                        if "showFig" in plot2DNet and not isinstance(
                            plot2DNet["showFig"], bool
                        ):
                            errorMessages.append(
                ...
                if "nTE" in analysis:
                    ...
                    if not isinstance(nTE, dict) and not isinstance(nTE, bool):
                        ...
                        if "numShuffle" in nTE and not isinstance(
                            nTE["numShuffle"], int
                        ):
                            errorMessages.append(
                ...
                if "granger" in analysis:
                    ...
                    if not isinstance(granger, dict) and not isinstance(granger, bool):
                        ...
                        if "cells1" in granger and not isinstance(
                            granger["cells1"], list
                        ):
                            errorMessages.append(
                        ...
                        if "cells2" in granger and not isinstance(
                            granger["cells2"], list
                        ):
                            errorMessages.append(
                        ...
                        if "spks1" in granger and not isinstance(
                            granger["spks1"], list
                        ):
                            errorMessages.append(
                        ...
                        if "spks2" in granger and not isinstance(
                            granger["spks2"], list
                        ):
                            errorMessages.append(
                        ...
                        if "binSize" in granger and not isinstance(
                            granger["binSize"], int
                        ):
                            errorMessages.append(
                        ...
                        if "plotFig" in plot2DNet and not isinstance(
                            plot2DNet["plotFig"], bool
                        ):
                            errorMessages.append(
...
class TestObj(object):
    ...
    def __init__(self):
        ...
        self.testValueList = (
            []
        )  # could be restricted list like ['cuboid','ellipsoid','cylinder']
        ...
        self.compareValueDataType = (
            ""  # data type of compare value ( string or list or dict
        )
        self.compareValueType = (
            ""  # eval (if compareValueString is string or int or float
        )
...
class SimTestObj(object):
    ...
    def loadSimConfigTests(self):
        ...
        testObj.messageText = [
            "simConfig->'hParams':hParams is not a dict.",
            "simConfig->'hParams':",
        ]
        ...
        testObj.messageText = [
            "simConfig->'cacheEfficient':cacheEfficient is not a boolean."
        ]
        testObj.errorMessageLevel = [MESSAGE_TYPE_ERROR]
        ...
        testObj.messageText = [
            "simConfig->'seeds':seeds is not a dict.",
            "SimConfig->'seeds':",
        ]
        testObj.errorMessageLevel = [MESSAGE_TYPE_ERROR, MESSAGE_TYPE_ERROR]
        ...
        testObj.testTypes = [TEST_TYPE_IS_BOOL]
        testObj.messageText = [
            "simConfig->'createNEURONObj':createNEURONObj is not a boolean."
        ]
        testObj.errorMessageLevel = [MESSAGE_TYPE_ERROR]
        ...
        testObj.testTypes = [TEST_TYPE_IS_BOOL]
        testObj.messageText = [
            "simConfig->'createPyStruct':createPyStruct is not a boolean."
        ]
        testObj.errorMessageLevel = [MESSAGE_TYPE_ERROR]
        ...
        testObj.testTypes = [TEST_TYPE_IS_BOOL]
        testObj.messageText = [
            "simConfig->'gatherOnlySimData':gatherOnlySimData is not a boolean."
        ]
        testObj.errorMessageLevel = [MESSAGE_TYPE_ERROR]
        ...
        testObj.testTypes = [TEST_TYPE_IS_FLOAT]
        testObj.messageText = [
            "simConfig->'printRunTime':printRunTime is not a boolean."
        ]
        testObj.errorMessageLevel = [MESSAGE_TYPE_ERROR]
        ...
        testObj.testTypes = [TEST_TYPE_IS_BOOL]
        testObj.messageText = [
            "simConfig->'printPopAvgRates':printPopAvgRates is not a boolean."
        ]
        testObj.errorMessageLevel = [MESSAGE_TYPE_ERROR]
        ...
        testObj.testTypes = [TEST_TYPE_IS_BOOL]
        testObj.messageText = [
            "simConfig->'includeParamsLabel':includeParamsLabel is not a boolean."
        ]
        testObj.errorMessageLevel = [MESSAGE_TYPE_ERROR]
        ...
        testObj.testTypes = [TEST_TYPE_IS_LIST]
        testObj.messageText = [
            "SimConfig->'backupCfgFile':backupCfgFile is not a list."
        ]
        testObj.errorMessageLevel = [MESSAGE_TYPE_ERROR]
    ...
    def loadPopTests(self):
        ...
        testObj.messageText = [
            "popParams->'cellModel': No cellModel specified in population parameters."
        ]
        testObj.errorMessageLevel = [MESSAGE_TYPE_ERROR]
        ...
        testObj.messageText = [
            "popParams: One of the following must be specified in parameters: "
        ...
        testObj.messageText = [
            "popParams->'xRange': xRange invalid.",
            "popParams->'xRange': xRange not in range.",
        ]
        ...
        testObj.messageText = [
            "popParams->'yRange': yRange invalid.",
            "popParams->'yRange': yRange not in range.",
        ]
        testObj.errorMessageLevel = [MESSAGE_TYPE_ERROR, MESSAGE_TYPE_ERROR]
        ...
        testObj.testValueRange = "[0,self.netParams.sizeX]"
        testObj.messageText = [
            "popParams->'zRange': zRange invalid.",
            "popParams->'zRange': zRange not in range.",
        ]
        testObj.errorMessageLevel = [MESSAGE_TYPE_ERROR, MESSAGE_TYPE_ERROR]
    ...
    def loadCellTests(self):
        ...
        testObj.messageText = [
            "cellParams->'secs'->'geom': Geom is not specified in section "
        ]
        testObj.errorMessageLevel = [MESSAGE_TYPE_ERROR]
    ...
    def runStimSourceTests(self):
        ...
        for testName, stimSourceTestObj in list(
            self.testParamsMap["stimSource"].items()
        ):
    ...
    def runStimTargetTests(self):
        ...
        for testName, stimTargetTestObj in list(
            self.testParamsMap["stimTarget"].items()
        ):
            ...
            if testType == TEST_TYPE_EXISTS:
                ...
                if isinstance(params, dict):
                    ...
                    for paramLabel, paramValues in list(params.items()):
                        ...
                        try:
                            self.testTypeObj.testExists(
                                testObj.testParameterValue, paramValues
                            )
                            if self.verboseFlag:
                                print(
                                    (
                ...
                if isinstance(params, dict):
                    for paramLabel, paramValues in list(params.items()):
                        ...
                        try:
                            self.testTypeObj.testExistsInList(
                                testObj.testParameterValueList, paramValues
                            )
                            if self.verboseFlag:
                                print(
                                    (
                    ...
                    for paramLabel, paramValues in list(params.items()):
                        ...
                        try:
                            ...
                            if (
                                ...
                                and testObj.testParameterValue1
                                in paramValues[testObj.testParameterValue]
                                and paramValues[testObj.testParameterValue][
                                    testObj.testParameterValue1
                                ]
                            ):
                                self.testTypeObj.testExistsInDict(
                                    paramValues[testObj.testParameterValue][
                                        testObj.testParameterValue1
                                    ],
                ...
                if isinstance(params, dict):
                    ...
                    for paramLabel, paramValues in list(params.items()):
                        ...
                        try:
                            self.testTypeObj.testIsValidRange(
                                testObj.testParameterValue, paramValues
                            )
                            if self.verboseFlag:
                                print(
                ...
                if isinstance(params, dict):
                    ...
                    for paramLabel, paramValues in list(params.items()):
                        ...
                        try:
                            if testObj.testParameterValue in paramValues:
                                self.testTypeObj.testIsBoolean(
                                    paramValues[testObj.testParameterValue]
                                )
                                if self.verboseFlag:
                                    print(
            ...
            #                 print str(testObj.errorMessageLevel[testIndex]) + ": " + str(testObj.messageText[testIndex]) + str(e[0])
                ...
                if isinstance(params, dict):
                    ...
                    try:
                        paramName = eval(testObj.testParameterValue)
                        self.testTypeObj.testIsValueList(
                            paramName, testObj.testValueList
                        )
                        if self.verboseFlag:
                            print(
                                (
                ...
                try:
                    ...
                    if paramDict != "":
                        self.testTypeObj.testDictKeyValidValue(
                            paramDict, testObj.testValueList
                        )
                        if self.verboseFlag:
                            print(
                                (
                        ...
                        try:
                            ...
                            for errorIndex, errorMessage in enumerate(errorMessages):
                                ...
                                if mechsValidFlagList[errorIndex]:
                                    ...
                                    print(
                                        (str(MESSAGE_TYPE_ERROR) + ": " + errorMessage)
                                    )
                            ...
                            print(
                                (
                                    ...
                                    + ": Mechanism specified is invalid."
                                )
                            )
                ...
                if isinstance(params, dict):
                    for paramLabel, paramValues in list(params.items()):
                        try:
                            errorMessages = self.testTypeObj.testValidPointps(
                                paramValues
                            )
                ...
                if isinstance(params, dict):
                    for paramLabel, paramValues in list(params.items()):
                        try:
                            self.testTypeObj.testValidSynMechs(
                                testObj.testParameterValue, paramValues, self.netParams
                            )
                            if self.verboseFlag:
                ...
                if isinstance(params, dict):
                    for paramLabel, paramValues in list(params.items()):
                        try:
                            ...
                            errorMessage = self.testTypeObj.testValidSecLists(
                                paramValues
                            )
                ...
                if isinstance(params, dict):
                    for paramLabel, paramValues in list(params.items()):
                        try:
                            ...
                            errorMessage = self.testTypeObj.testTypeHierarchy(
                                paramValues
                            )
                ...
                if isinstance(params, dict):
                    for paramLabel, paramValues in list(params.items()):
                        try:
                            ...
                            errorMessages = self.testTypeObj.testValidConnShape(
                                paramValues
                            )
                ...
                if isinstance(params, dict):
                    for paramLabel, paramValues in list(params.items()):
                        try:
                            ...
                            errorMessages = self.testTypeObj.testValidConnPlasticity(
                                paramValues
                            )
                ...
                if isinstance(params, dict):
                    for paramLabel, paramValues in list(params.items()):
                        try:
                            ...
                            errorMessages = self.testTypeObj.testValidStimSource(
                                paramValues
                            )
                ...
                if isinstance(params, dict):
                    for paramLabel, paramValues in list(params.items()):
                        try:
                            ...
                            errorMessages = self.testTypeObj.testValidStimTarget(
                                paramValues, self.netParams
                            )
                ...
                if isinstance(params, dict):
                    for paramLabel, paramValues in list(params.items()):
                        try:
                            ...
                            errorMessages = self.testTypeObj.testValidSpikeGenLoc(
                                paramValues
                            )
                ...
                if isinstance(params, dict):
                    for paramLabel, paramValues in list(params.items()):
                        try:
                            ...
                            stimValid, errorMessages = self.testTypeObj.testValidStim(
                                paramValues
                            )
