    ...
    if "net" in data and "cells" in data["net"] and "pops" in data["net"]:
        ...
        if loadNow:
            ...
            if compactConnFormat:
                compactToLongConnFormat(
                    data["net"]["cells"], compactConnFormat
        ...
        if instantiate:
                ...
                if hasattr(sim, "rank"):
                    if isinstance(instantiate, list):
                        cellsNode = [
                            data["net"]["cells"][i]
                            for i in range(
                                int(sim.rank), len(data["net"]["cells"]), sim.nhosts
                            )
                            if i in instantiate
                        ]
                    else:
                    ...
                    if isinstance(instantiate, list):
                        cellsNode = [
                            data["net"]["cells"][i]
                            for i in range(0, len(data["net"]["cells"]), 1)
                            if i in instantiate
                        ]
                    else:
                        cellsNode = [
                ...
                if sim.cfg.createPyStruct:
                    ...
                    for cellLoad in cellsNode:
                        ...
                        try:
                            ...
                            if sim.cfg.verbose:
                                print(" Unable to load cell secs")
                    ...
                    print(
                        (
                            "  Created %d stims"
                            % (sum([len(c.stims) for c in sim.net.cells]))
                        )
                    )
            except:
                    ...
                    if sim.cfg.createNEURONObj:
                        ...
                        for cell in sim.net.cells:
                            ...
                            cell.createNEURONObj(
                                prop
                            )  # use same syntax as when creating based on high-level specs
                        ...
                        print(
                            (
                                "  Added NEURON objects to %d cells"
                                % (len(sim.net.cells))
                            )
                        )
                except:
            ...
            if loadNow and sim.cfg.timing:  # if sim.rank == 0 and sim.cfg.timing:
                ...
                print(
                    (
                        "  Done; re-instantiate net time = %0.2f s"
                        % sim.timingData["loadNetTime"]
                    )
                )
    else:
    ...
    try:
        for cell in cells:
            for iconn, conn in enumerate(cell["conns"]):
                cell["conns"][iconn] = {
                    key: conn[index] for key, index in formatIndices.items()
                }
        return cells
    except:
...
def ijsonLoad(
    ...
    if saveTags and tags:
        outFilename = (
            saveTags
            if isinstance(saveTags, basestring)
            else "filename"[:-4] + "_tags.json"
        )
    ...
    if saveConns and conns:
        outFilename = (
            saveConns
            if isinstance(saveConns, basestring)
            else "filename"[:-4] + "_conns.json"
        )
