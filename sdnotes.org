...
- Modified importCell so it reads sectionLists with original variable names; and modified input arguments so now pass
  cellRule as 1st arg -- so can add 'sections' and 'sectionLists' to cellRule dict
...
-- maybe move 'synMechs' out, to same level as 'sections' ? 
-- add to sectionList !?
...
** change probConn so exact probability not based on random (paper)
- selected cells still based on random
- but always same exact percentage
...
** Add connecitity patterns for individual cells
- eg. LGN -> V1 patterns
- shared weights across input image
