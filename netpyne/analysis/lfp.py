...
def plotLFP(
    ...
    if "timeSeries" in plots:
        ...
        for i, elec in enumerate(electrodes):
            if elec == "avg":
                ...
                color = "k"
                lw = 1.0
            elif isinstance(elec, Number) and (
                inputLFP is not None or elec <= sim.net.recXElectrode.nsites
            ):
                lfpPlot = lfp[:, elec]
            ...
            plt.plot(
                t[0 : len(lfpPlot)], -lfpPlot + (i * ydisp), color=color, linewidth=lw
            )
            if len(electrodes) > 1:
                plt.text(
        ...
        round_to_n = (
            lambda x, n, m: int(
                np.ceil(round(x, -int(np.floor(np.log10(abs(x)))) + (n - 1)) / m)
            )
            * m
        )
    ...
    if "PSD" in plots:
        ...
        plt.tight_layout()
        plt.suptitle(
            "LFP Power Spectral Density", fontsize=fontSize, fontweight="bold"
        )  # add yaxis in opposite side
    ...
    if "spectrogram" in plots:
        ...
        if transformMethod == "morlet":
            ...
            f = (
                ...
                else np.array(range(minFreq, maxFreq + 1, stepFreq))
        ...
        # FFT transform method
            ...
            for i, elec in enumerate(electrodes):
                plt.subplot(np.ceil(len(electrodes) / numCols), numCols, i + 1)
                plt.pcolormesh(
                    x_mesh, y_mesh, spec[i], cmap=cm.viridis, vmin=vmin, vmax=vmax
                )
    ...
    if "locations" in plots:
        ...
        for cell in sim.net.compartCells:
            trSegs = list(
                np.sum(
                    sim.net.recXElectrode.getTransferResistance(cell.gid) * 1e3, axis=0
                )
    ...
    if "spectrogram" in plots:
        outputData.update(
            {"spec": spec, "t": t_spec * 1000.0, "freqs": f[f <= maxFreq]}
        )
