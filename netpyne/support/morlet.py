...
def Morlet(sampr, freq, width=7.0):
    ...
    return (
        A
        * np.exp(-(t ** 2.0) / (2.0 * st ** 2.0))
        * np.exp(1.0j * 2.0 * np.pi * freq * t)
    )
...
class MorletSpec:
    ...
    def plot_to_ax(self, ax_spec, dt):
        # plots spec to axis
        pc = ax_spec.imshow(
            self.TFR, aspect="auto", origin="upper", cmap=plt.get_cmap("jet")
        )
        return pc
    ...
    def transform(self, tsvec, getphase=False):
        ...
        sig = sps.detrend(
            tsvec - np.mean(tsvec)
        )  # subtract mean and (linear) detrend the input time series
        ...
        if getphase:
            ...
            for j, freq in enumerate(self.f):
                self.TFR[j, :], self.PHS[j, :] = MorletVec(
                    sig, self.sampr, freq, self.width, getphase=True
                )
        else:
            for j, freq in enumerate(self.f):
                self.TFR[j, :] = MorletVec(
                    sig, self.sampr, freq, self.width, getphase=False
                )
