...
- use params with initial underscore '_var' to indicate used by netpyne, not mechanism/pointprocess variable
** Distinguish syns from other point processes 
- no way to do it
- assume 'syn' in name -- establish naming convention

- syn: .is_netcon_target() = True; .has_net_event(i) = False; .is_artificial()
...
** timing
** Test Friesen cell, adaptive exp cell and IntFire1,2,3,4
