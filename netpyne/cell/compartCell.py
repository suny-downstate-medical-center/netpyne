...
class CompartCell(Cell):
    ...
    def create(self):
        ...
        for (
            propLabel,
            prop,
        ) in sim.net.params.cellParams.items():  # for each set of cell properties
            conditionsMet = 1
            if "conds" in prop and len(prop["conds"]) > 0:
                for (condKey, condVal) in prop[
                    "conds"
                ].items():  # check if all conditions are met
                    if isinstance(condVal, list):
                        if isinstance(condVal[0], Number):
                            if (
                                self.tags.get(condKey) < condVal[0]
                                or self.tags.get(condKey) > condVal[1]
                            ):
                                conditionsMet = 0
                                break
                ...
                self.tags["cellType"] != propLabel
            ):  # simplified method for defining cell params (when no 'conds')
                conditionsMet = False
        ...
        for sectName, sectParams in prop["secs"].items():
            ...
            if "mechs" in sectParams:
                for mechName, mechParams in sectParams["mechs"].items():
                    ...
                    for (
                        mechParamName,
                        mechParamValue,
                    ) in mechParams.items():  # add params of the mechanism
            ...
            if "ions" in sectParams:
                for ionName, ionParams in sectParams["ions"].items():
                    ...
                    for (
                        ionParamName,
                        ionParamValue,
                    ) in ionParams.items():  # add params of the ion
            ...
            if "pointps" in sectParams:
                for pointpName, pointpParams in sectParams["pointps"].items():
                    ...
                    for (
                        pointpParamName,
                        pointpParamValue,
                    ) in pointpParams.items():  # add params of the mechanism
            ...
            if "geom" in sectParams:
                for geomParamName, geomParamValue in sectParams["geom"].items():
                    if "geom" not in sec:
                        sec["geom"] = Dict()
                    if not type(geomParamValue) in [
                        list,
                        dict,
                    ]:  # skip any list or dic params
    ...
    def __dipoleInsert(self, secName, sec):
        ...
        for i in range(len(loc)):
            ...
            if i > 0:
                h.setpointer(
                    sec["hObj"](loc[i - 1])._ref_v, "pv", sec["hObj"](loc[i]).dipole
                )
            else:
            ...
            h.setpointer(
                self.dipole["hRef"]._ref_x[0], "Qtotal", sec["hObj"](loc[i]).dipole
            )
    ...
    def createNEURONObj(self, prop):
        ...
        for sectName, sectParams in prop["secs"].items():
            ...
            if "hObj" not in self.secs[sectName] or self.secs[sectName]["hObj"] in [
                None,
                {},
                [],
            ]:
                self.secs[sectName]["hObj"] = h.Section(
                    name=sectName, cell=self
                )  # create h Section object
            sec = self.secs[sectName]  # pointer to section
            ...
            if "mechs" in sectParams:
                mechsInclude = {
                    k: v
                    for k, v in sectParams["mechs"].items()
                    if k not in excludeMechs
                }
                for mechName, mechParams in mechsInclude.items():
                    ...
                    for (
                        ...
                        for iseg, seg in enumerate(
                            sec["hObj"]
                        ):  # set mech params for each segment
                            ...
                            if (
                                mechParamValueFinal is not None
            ...
            if "ions" in sectParams:
                for ionName, ionParams in sectParams["ions"].items():
                    ...
                    try:
                        ...
                        if sim.cfg.verbose:
                            print(
                                "# Error inserting %s ion in %s section!"
                                % (ionName, sectName)
                            )
                        continue
                    for (
                        ionParamName,
                        ionParamValue,
                    ) in ionParams.items():  # add params of the mechanism
                        ionParamValueFinal = ionParamValue
                        for iseg, seg in enumerate(
                            sec["hObj"]
                        ):  # set ion params for each segment
                            ...
                            if ionParamName == "e":
                                ...
                                h(
                                    "%so0_%s_ion = %s"
                                    % (ionName, ionName, ionParamValueFinal)
            ...
            if "synMechs" in sectParams:
                for synMech in sectParams["synMechs"]:
                    if "label" in synMech and "loc" in synMech:
                        ...
                        self.addSynMechNEURONObj(
                            synMech, synMechParams, sec, synMech["loc"]
                        )
            ...
            if "pointps" in sectParams:
                for pointpName, pointpParams in sectParams["pointps"].items():
                    ...
                    loc = (
                        pointpParams["loc"] if "loc" in pointpParams else 0.5
                    )  # set location
                    sec["pointps"][pointpName]["hObj"] = pointpObj(
                        loc, sec=sec["hObj"]
                    )  # create h Pointp object (eg. h.Izhi2007b)
                    for (
                        pointpParamName,
                        pointpParamValue,
                    ) in pointpParams.items():  # add params of the point process
                        if pointpParamValue == "gid":
                            pointpParamValue = self.gid
                        if (
                            pointpParamName
                            ...
                            and not pointpParamName.startswith("_")
                        ):
                            setattr(
                                sec["pointps"][pointpName]["hObj"],
                                pointpParamName,
                                pointpParamValue,
                            )
                    if "params" in self.tags.keys():  # modify cell specific params
                        for pointpParamName, pointpParamValue in self.tags[
                            "params"
                        ].items():
                            setattr(
                                sec["pointps"][pointpName]["hObj"],
        ...
        # set topology
        for sectName, sectParams in prop[
            "secs"
        ].items():  # iterate sects again for topology (ensures all exist)
        ...
        if sim.cfg.recordDipoles:
            ...
            self.dipole = {
                "hRef": h.Vector(1)
            }  # _ref_[0]} #h._ref_dpl_ref}  #h.Vector(1)
            ...
            for sectName, sectParams in prop["secs"].items():
                ...
                if "mechs" in sectParams and "dipole" in sectParams["mechs"]:
                    self.__dipoleInsert(
                        sectName, sec
                    )  # add dipole mechanisms to each section
    ...
    def addSynMechNEURONObj(self, synMech, synMechParams, sec, loc):
        if not synMech.get("hObj"):  # if synMech doesn't have NEURON obj, then create
            ...
            synMech["hObj"] = synObj(
                loc, sec=sec["hObj"]
            )  # create h Syn object (eg. h.Exp2Syn)
            for (
                synParamName,
                synParamValue,
            ) in synMechParams.items():  # add params of the synaptic mechanism
                if synParamName not in ["label", "mod", "selfNetCon", "loc"]:
                    setattr(synMech["hObj"], synParamName, synParamValue)
                elif (
                    synParamName == "selfNetcon"
                ):  # create self netcon required for some synapses (eg. homeostatic)
                    secLabelNetCon = synParamValue.get("sec", "soma")
                    locNetCon = synParamValue.get("loc", 0.5)
                    secNetCon = self.secs.get(secLabelNetCon, None)
                    synMech["hObj"] = h.NetCon(
                        secNetCon["hObj"](locNetCon)._ref_v,
                        synMech[""],
                        sec=secNetCon["hObj"],
                    )
    ...
    def addStimsNEURONObj(self):
        ...
        for stimParams in self.stims:
            if stimParams["type"] == "NetStim":
                ...
                stim = getattr(h, stimParams["type"])(
                    self.secs[stimParams["sec"]]["hObj"](stimParams["loc"])
                )
                stimProps = {
                    k: v
                    for k, v in stimParams.items()
                    if k not in ["label", "type", "source", "loc", "sec", "hObj"]
                }
                for (
                    stimPropName,
                    stimPropValue,
                ) in stimProps.items():  # set mechanism internal stimParams
    ...
    def addConnsNEURONObj(self):
        ...
        for conn in self.conns:
            ...
            if sim.cfg.oneSynPerNetcon:
                ...
                synMech = next(
                    (
                        ...
                        if synMech["label"] == conn["synMech"]
                        and synMech["loc"] == conn["loc"]
                    ),
                    None,
                )
            ...
            # Add plasticity
            if conn.get("plast"):
                self._addConnPlasticity(
                    conn["plast"], self.secs[conn["sec"]], netcon, 0
                )
        ...
        if self.secs:
            if sim.cfg.createNEURONObj:
                ...
                sec = next(
                    (
                        secParams
                        for secName, secParams in self.secs.items()
                        if "spikeGenLoc" in secParams
                    ),
                    None,
                ...
                if sec:
                    loc = sec[
                        "spikeGenLoc"
                    ]  # get location of spike generator within section
                else:
                    ...
                    sec = next(
                        ...
                        self.secs[list(self.secs.keys())[0]],
                    )  # root sec (no parents)
                    loc = 0.5
                nc = None
                if (
                    "pointps" in sec
                ):  # if no syns, check if point processes with 'vref' (artificial cell)
                    for pointpName, pointpParams in sec["pointps"].items():
                        if "vref" in pointpParams:
                            nc = h.NetCon(
                                getattr(
                                    sec["pointps"][pointpName]["hObj"],
                                    "_ref_" + pointpParams["vref"],
                                ),
                                None,
                                sec=sec["hObj"],
                            )
                ...
                threshold = (
                    ...
                    else sim.net.params.defaultThreshold
                )
                nc.threshold = threshold
                sim.pc.cell(
                    self.gid, nc, 1
                )  # associate a particular output stream of events
    ...
    def addSynMech(self, synLabel, secLabel, loc):
        ...
        if synMechParams and sec:  # if both the synMech and the section exist
            if sim.cfg.createPyStruct and sim.cfg.addSynMechs:
                ...
                if (
                    not synMech
                ):  # if synMech not in section, or need multiple synMech per section, then create
        ...
        if conditionsMet:
            for secLabel, sec in self.secs.items():
                for synMech in sec["synMechs"]:
                    ...
                    if "conds" in params:
                        for (condKey, condVal) in params[
                            ...
                            if condKey == "sec":
                                ...
                                if (
                                    synMech.get(condKey) < condVal[0]
                                    or synMech.get(condKey) > condVal[1]
                                ):
                                    conditionsMet = 0
                                    break
                            elif isinstance(condVal, list) and isinstance(
                    ...
                    if (
                        conditionsMet
                    ):  # if all conditions are met, set values for this cell
                        exclude = [
                            "conds",
                            "cellConds",
                            "label",
                        ...
                        for synParamName, synParamValue in {
                            ...
                            if sim.cfg.createNEURONObj:
                                try:
                                    setattr(
                                        synMech["hObj"], synParamName, synParamValue
                                    )
                                except:
                                    print(
                                        "Error setting %s=%s on synMech"
                                        % (synParamName, str(synParamValue))
                                    )
    ...
    def addConn(self, params, netStimParams=None):
        ...
        if not pointp:  # check not a point process
            synMechs, synMechSecs, synMechLocs = self._setConnSynMechs(
                params, secLabels
            )
            if synMechs == -1:
                return
            ...
            for i, (sec, loc) in enumerate(zip(synMechSecs, synMechLocs)):
                if "weightNorm" in self.secs[sec] and isinstance(
                    self.secs[sec]["weightNorm"], list
                ):
                    nseg = self.secs[sec]["geom"]["nseg"]
                    weights[i] = (
                        weights[i]
                        * self.secs[sec]["weightNorm"][int(round(loc * nseg)) - 1]
                    )
        ...
        for i in range(params["synsPerConn"]):
            ...
            if (
                params.get("gapJunction", False) == True
            ):  # only run for post gap junc (not pre)
                if hasattr(sim, "rank"):
                    preGapId = (
                        1e9 * sim.rank + sim.net.lastGapId
                    )  # global index for presyn gap junc
                else:
                ...
                if not getattr(sim.net, "preGapJunctions", False):
                    sim.net.preGapJunctions = (
                        []
                    )  # if doesn't exist, create list to store presynaptic cell gap junctions
                preGapParams = {
                    "gid": params["preGid"],
                    "preGid": self.gid,
                ...
                sim.net.preGapJunctions.append(
                    preGapParams
                )  # add conn params to add pre gap junction later

            # Python Structure
            if sim.cfg.createPyStruct:
                connParams = {
                    k: v for k, v in params.items() if k not in ["synsPerConn"]
                }
            ...
            if sim.cfg.createNEURONObj:
                # gap junctions
                if params.get("gapJunction", "False") in [
                    True,
                    "pre",
                    "post",
                ]:  # create NEURON obj for pre and post
                    synMechs[i]["hObj"].weight = weights[i]
                    sourceVar = self.secs[synMechSecs[i]]["hObj"](synMechLocs[i])._ref_v
                    targetVar = synMechs[i][
                        "hObj"
                    ]._ref_vpeer  # assumes variable is vpeer -- make a parameter
                ...
                # connections using NetCons
                    ...
                    if netStimParams:
                        netcon = h.NetCon(
                            netstim, postTarget
                        )  # create Netcon between netstim and target
                    else:
                    ...
                    self.conns[-1][
                        "hObj"
                    ] = netcon  # add netcon object to dict in conns list
                ...
                if "shape" in params and params["shape"]:
                    ...
                    # Default shape
                    pulsetype = (
                        params["shape"]["pulseType"]
                        if "pulseType" in params["shape"]
                        else "square"
                    )
                    pulsewidth = (
                        params["shape"]["pulseWidth"]
                        if "pulseWidth" in params["shape"]
                        else 100.0
                    )
                    pulseperiod = (
                        params["shape"]["pulsePeriod"]
                        if "pulsePeriod" in params["shape"]
                        else 100.0
                    )
                    ...
                    if "switchOnOff" not in params["shape"]:
                        ...
                        if not params["shape"]["switchOnOff"] == sorted(
                            params["shape"]["switchOnOff"]
                        ):
                            raise Exception(
                                "On-off switching times for a particular stimulus are not monotonic"
                            )
                    ...
                    self.conns[-1]["shapeTimeVec"] = h.Vector().from_python(
                        temptimevecs
                    )
                    self.conns[-1]["shapeWeightVec"] = h.Vector().from_python(
                        tempweightvecs
                    )
                    ...
                    if cell:
                        for (condKey, condVal) in params[
                            ...
                            if isinstance(condVal, list) and isinstance(
                                ...
                                if (
                                    cell.tags.get(condKey) < condVal[0]
                                    or cell.tags.get(condKey) > condVal[1]
                                ):
                                    conditionsMet = 0
                                    break
                            elif isinstance(condVal, list) and isinstance(
            ...
            if conditionsMet:  # if all conditions are met, set values for this cell
                if sim.cfg.createPyStruct:
                    for paramName, paramValue in {
                        ...
                        if k not in ["conds", "preConds", "postConds"]
                    }.items():
            ...
            for stim in self.stims:
                ...
                if "conds" in params:
                    for (condKey, condVal) in params[
                        ...
                        if isinstance(condVal, list) and isinstance(condVal[0], Number):
                            if (
                                stim.get(condKey) < condVal[0]
                                or stim.get(condKey) > condVal[1]
                            ):
                                conditionsMet = 0
                                break
                        elif isinstance(condVal, list) and isinstance(
                ...
                if conditionsMet:  # if all conditions are met, set values for this cell
                    if (
                        stim["type"] == "NetStim"
                    ):  # for netstims, find associated netcon
                        conn = next(
                            (
                                conn
                                for conn in self.conns
                                if conn["source"] == stim["source"]
                            ),
                            None,
                        )
                    if sim.cfg.createPyStruct:
                        for paramName, paramValue in {
                            k: v
                            for k, v in params.items()
                            if k not in ["conds", "cellConds"]
                        }.items():
                            if stim["type"] == "NetStim" and paramName in [
                                "weight",
                                "delay",
                            ]:
                                conn[paramName] = paramValue
                            ...
                            try:
                                if stim["type"] == "NetStim":
                                    if paramName == "weight":
                                        ...
                                        setattr(
                                            stim["hObj"], "interval", stim["interval"]
                                        )
    ...
    def addStim(self, params):
        ...
        if not params["sec"] or (
            ...
            if "soma" in self.secs:
                ...
                params["sec"] = list(self.secs.keys())[
                    0
                ]  # if no 'soma', use first sectiona available
            else:
                if sim.cfg.verbose:
                    print(
                        "  Error: no Section available on cell gid=%d to add stim"
                        % (self.gid)
                    )
                return
        ...
        if params["type"] == "NetStim":
            ...
            stimParams = {
                k: v
                for k, v in params.items()
                if k not in ["type", "source", "loc", "sec", "label"]
            }
            stringParams = ""
            for (
                stimParamName,
                stimParamValue,
            ) in stimParams.items():  # set mechanism internal params
                if isinstance(stimParamValue, list):
                    ...
                    stringParams = (
                        stringParams + ", " + stimParamName + "=" + str(stimParamValue)
                    )
            ...
            for (
                ...
                if isinstance(stimParamValue, list):
                    print(
                        "Can't set point process paramaters of type vector eg. VClamp.amp[3]"
                    )
                    pass
                    # setattr(stim, stimParamName._ref_[0], stimParamValue[0])
                elif (
                    "originalFormat" in params
                    and stimParamName == "originalFormat"
                    and params["originalFormat"] == "NeuroML2_stochastic_input"
                ):
                    if sim.cfg.verbose:
                        print(("   originalFormat: %s" % (params["originalFormat"])))

                    rand = h.Random()
                    stim_ref = params["label"][
                        : params["label"].rfind(self.tags["pop"])
                    ]

                    # e.g. Stim3_2_popPyrS_2_soma_0_5 -> 2
                    index_in_stim = int(stim_ref.split("_")[-2])
                    stim_id = stim_ref.split("_")[0]
                    sim._init_stim_randomizer(
                        rand, stim_id, index_in_stim, sim.cfg.seeds["stim"]
                    )
                    rand.negexp(1)
    ...
    def _setConnSections(self, params):
        ...
        if not params.get("sec") or (
            ...
            and not params.get("sec")
            in list(self.secs.keys()) + list(self.secLists.keys())
        ):
            if sim.cfg.verbose:
                print(
            ...
            if "soma" in self.secs:
                ...
                if sim.cfg.verbose:
                    print(
                        "  Error: no Section available on cell gid=%d to add connection"
                        % (self.gid)
                    )
        ...
        # if sectionList or list of sections
            ...
            secList = (
                list(params["sec"])
                if isinstance(params["sec"], list)
                else list(self.secLists[params["sec"]])
            )
            secLabels = []
            for i, section in enumerate(secList):
                if (
                    section not in self.secs
                ):  # remove sections that dont exist; and corresponding weight and delay
                    if sim.cfg.verbose:
                        print(
    ...
    def _setConnWeights(self, params, netStimParams, secLabels):
        ...
        if netStimParams:
            ...
            and sim.net.params.scaleConnWeightModels.get(self.tags["cellModel"], None)
            is not None
        ):
    ...
    def _setConnPointP(self, params, secLabels, weightIndex):
        ...
        if (
            ...
            for pointpName, pointpParams in self.secs[secLabels[0]]["pointps"].items():
                if (
                    "vref" in pointpParams
                ):  # if includes vref param means doesn't use Section v or synaptic mechanisms
                    pointp = pointpName
                    if "synList" in pointpParams:
                        if params.get("synMech") in pointpParams["synList"]:
                            if isinstance(params.get("synMech"), list):
                                weightIndex = [
                                    pointpParams["synList"].index(synMech)
                                    for synMech in params.get("synMech")
                                ]
                            else:
        ...
        if (
            pointp and params["synsPerConn"] > 1
        ):  # only single synapse per connection rule allowed
            if sim.cfg.verbose:
                print(
    ...
    def _setConnSynMechs(self, params, secLabels):
        ...
        if not params.get("synMech"):
            if (
                sim.net.params.synMechParams
            ):  # if no synMech specified, but some synMech params defined
                ...
                if sim.cfg.verbose:
                    print(
                        "  Error: no synaptic mechanisms available to add conn on cell gid=%d "
                        % (self.gid)
                    )
        ...
        if synsPerConn > 1:  # if more than 1 synapse
            if len(secLabels) == 1:  # if single section, create all syns there
                ...
                if isinstance(params["loc"], list):
                    ...
                    synMechLocs = [
                        i * (1.0 / synsPerConn) + 1.0 / synsPerConn / 2
                ...
                if sim.cfg.distributeSynsUniformly:
                    ...
                    if not sim.cfg.connRandomSecFromList and synsPerConn == len(
                        ...
                        if isinstance(params["loc"], list):
                            if (
                                len(params["loc"]) == synsPerConn
                            ):  # list of locs matches num syns
                                synMechLocs = params["loc"]
                        ...
                        if (
                            sim.cfg.connRandomSecFromList
                            and len(synMechSecs) >= synsPerConn
                        ):
                            ...
                            preGid = (
                                params["preGid"]
                                if isinstance(params["preGid"], int)
                                else 0
                            )
                            rand.Random123(
                                sim.hashStr("connSynMechsSecs"), self.gid, preGid
                            )  # initialize randomizer

                            randSecPos = sim.net.randUniqueInt(
                                rand, synsPerConn, 0, len(synMechSecs) - 1
                            )
                            synMechSecs = [synMechSecs[i] for i in randSecPos]

                            if isinstance(params["loc"], list):
                                randLocPos = sim.net.randUniqueInt(
                                    rand, synsPerConn, 0, len(synMechLocs) - 1
                                )
                                synMechLocs = [synMechLocs[i] for i in randLocPos]
                            else:
                                randLoc = rand.uniform(0, 1)
                                synMechLocs = [
                                    rand.uniform(0, 1) for i in range(synsPerConn)
                                ]
                        else:
                            print(
            ...
            synMechSecs = secLabels
            synMechLocs = (
                params["loc"] if isinstance(params["loc"], list) else [params["loc"]]
            )
        ...
        synMechs = [
            self.addSynMech(
                synLabel=params["synMech"], secLabel=synMechSecs[i], loc=synMechLocs[i]
            )
            for i in range(synsPerConn)
        ]
        ...
        try:
            if "L" in self.secs[secList[0]]["geom"]:
                ...
                if sim.cfg.verbose:
                    print(
                        (
                            "  Section lengths not available to distribute synapses in cell %d"
                            % self.gid
                        )
                    )
            ...
            absLocs = [
                i * (totLength / numSyns) + totLength / numSyns / 2
                for i in range(numSyns)
            ]
            inds = [
                cumLengths.index(next(x for x in cumLengths if x >= absLoc))
                for absLoc in absLocs
            ]
            secs = [secList[ind] for ind in inds]
            locs = [
                (cumLengths[ind] - absLoc) / secLengths[ind]
                for absLoc, ind in zip(absLocs, inds)
            ]
        except:
            secs, locs = [], []
        return secs, locs
    ...
    def _addConnPlasticity(self, params, sec, netcon, weightIndex):
        ...
        if plasticity and sim.cfg.createNEURONObj:
            try:
                ...
                if (
                    plasticity["mech"] == "STDP"
                ):  # specific implementation steps required for the STDP mech
                    ...
                    pstcon.weight[
                        0
                    ] = -1  # Send postsynaptic spikes to the STDP adjuster
                    h.setpointer(
    ...
    def calcAbsSegCoords(self):
        ...
        p3dsoma = p3dsoma[
            np.newaxis
        ].T  # trasnpose 1d array to enable matrix calculation
    ...
    def updateShape(self):
        ...
        for sec in list(self.secs.values()):
            if (
                "geom" in sec and "pt3d" not in sec["geom"]
