...
def plotTraces(
    ...
    def plotFigPerTrace(subGids):
        for itrace, trace in enumerate(tracesList):
            figs["_trace_" + str(trace)] = plt.figure(
                figsize=figSize
            )  # Open a new figure
                    ...
                    if isinstance(fullTrace, dict):
                        if recordStep == "adaptive":
                            ...
                            for key in list(fullTrace.keys()):
                                t.append(
                                    np.array(
                                        sim.allSimData[trace]["cell_time_" + str(gid)][
                                            key
                                        ]
                                    )
                                )
                                t_indexes.append(
                                    t[-1]
                                    .__ge__(timeRange[0])
                                    .__and__(t[-1].__le__(timeRange[1]))
                                )
                            ...
                            t = np.arange(
                                timeRange[0], timeRange[1] + recordStep, recordStep
                            )
                    else:
                        if recordStep == "adaptive":
                            t = np.array(sim.allSimData[trace]["cell_time_" + str(gid)])
                            t_indexes = t.__ge__(timeRange[0]).__and__(
                                t.__le__(timeRange[1])
                            )
                            data = np.array(sim.allSimData[trace]["cell_" + str(gid)])[
                                t_indexes
                            ]
                            lenData = len(data)
                            t = t[t_indexes]
                ...
                if "cell_" + str(gid) in sim.allSimData[trace]:
                    ...
                    if recordStep == "adaptive":
                        if isinstance(data, list) and isinstance(
                            data[0], (list, np.array)
                        ):
                            for tl, dl in zip(t, data):
                                plt.plot(
                                    tl, dl, linewidth=1.5, color=color, label=trace
                                )
                        else:
                            plt.plot(t, data, linewidth=1.5, color=color, label=trace)
                    else:
                        plt.plot(
                            t[:lenData], data, linewidth=1.5, color=color, label=trace
                        )
                    plt.xlabel("Time (ms)", fontsize=fontsiz)
                    plt.ylabel(trace, fontsize=fontsiz)
                    plt.xlim(timeRange)
                    ...
                    if not overlay:
                        if (
                            not axis or axis == "off" and itrace == 0
                        ):  # if no axis, add scalebar
                            addScaleBar()
