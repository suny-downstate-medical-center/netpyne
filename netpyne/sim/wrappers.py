...
def distributedSimulate(filename=None, dataDir=None, includeLFP=True):
    ...
    sim.runSim()
    sim.saveDataInNodes(
        filename=filename, saveLFP=includeLFP, removeTraces=False, dataDir=dataDir
    )
...
def createSimulate(netParams=None, simConfig=None, output=False):
    ...
    (pops, cells, conns, stims, rxd, simData) = sim.create(
        netParams, simConfig, output=True
    )
    sim.simulate()
...
# Wrapper to create, simulate, and analyse network, while saving to master in intervals
# ------------------------------------------------------------------------------
def createSimulateAnalyzeDistributed(
    netParams, simConfig, output=False, filename=None, dataDir=None, includeLFP=True
):
    """
