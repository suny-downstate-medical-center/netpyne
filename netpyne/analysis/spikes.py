...
def calculateRate(include=["allCells", "eachPop"], peakBin=5, timeRange=None):
    ...
    for iplot, subset in enumerate(include):
        ...
        if peakBin:
            histo = np.histogram(
                spkts, bins=np.arange(timeRange[0], timeRange[1], peakBin)
            )
...
def plotRates(
    ...
    for i, timeRange in enumerate(timeRanges):
        labels, avg, peak = sim.analysis.calculateRate(
            include=include, peakBin=peakBin, timeRange=timeRange
        )
        avgs.append(avg)
        peaks.append(peak)
...
def plotRaster(
    ...
    if (
        ...
        orderBy = "popInd"
    elif isinstance(orderBy, basestring) and not isinstance(
        cells[0]["tags"][orderBy], Number
    ):
        orderBy = "gid"
    ...
    if len(cellGids) > 0:
        gidColors = {
            cell["gid"]: popColors[cell["tags"]["pop"]] for cell in cells
    ...
    if syncLines:
        ...
        plt.title(
            "cells=%i   syns/cell=%0.1f   rate=%0.1f Hz"
            % (numCells, connsPerCell, firingRate),
            fontsize=fontsiz,
        )
    # Axis
    ...
    if labels == "legend":
        ...
        tysOffset = [tysOffset[0]] + [
            tysOffset[i] + max(tysOffset[i + 1] - tysOffset[i], minSpacing)
            for i in range(len(tysOffset) - 1)
        ]
...
def plotSpikeHist(
    ...
    **kwargs
    ...
    if (
        len(include) < 5
    ):  # if apply tight_layout with many subplots it inverts the y-axis
        try:
            plt.tight_layout()
        except:
            pass
    ...
    if not axis:
        ...
        round_to_n = (
            lambda x, n, m: int(
                np.round(round(x, -int(np.floor(np.log10(abs(x)))) + (n - 1)) / m)
            )
            * m
        )
...
def plotSpikeStats(
    ...
    **kwargs
    ...
    for stat in stats:
            ...
            if stat in statDataIn:
                ...
                if graphType == "scatter":
                    if includeRate0:
                        ...
                        gids = set(spkinds)
                    ynorms = [
                        sim.net.allCells[int(gid)]["tags"]["ynorm"] for gid in gids
                    ]
        ...
        if graphType == "boxplot":
            meanpointprops = dict(
                marker=(5, 1, 0), markeredgecolor="black", markerfacecolor="white"
            )
        ...
        # histogram
            ...
            plt.xlabel(xlabel, fontsize=fontsiz)
            plt.ylabel(
                "Probability of occurrence" if density else "Frequency",
                fontsize=fontsiz,
            )
            xmax = binmax
            plt.xlim(histmin, xmax)
            plt.ylim(
                0, 1.1 * nmax if density else np.ceil(1.1 * nmax)
            )  # min(n[n>=0]), max(n[n>=0]))
        ...
        # scatter
            ...
            for i, (ynorms, data) in enumerate(zip(ynormsData, statData)):
                mean, binedges, _ = stats.binned_statistic(
                    ynorms, data, "mean", bins=bins
                )
                median, binedges, _ = stats.binned_statistic(
                    ynorms, data, "median", bins=bins
                )
                # p25 = lambda x: np.percentile(x, 25)
                # p75 = lambda x: np.percentile(x, 75)

                std, binedges, _ = stats.binned_statistic(
                    ynorms, data, "std", bins=bins
                )
                ...
                if kwargs.get("differentColor", None):
                    ...
                    plt.scatter(
                        ynorms,
                        data,
                        color=[0 / 255.0, 215 / 255.0, 255 / 255.0],
                        label=label,
                        s=2,
                    )  # [88/255.0,204/255.0,20/255.0]
        ...
        if saveFig:
            if isinstance(saveFig, basestring):
                ...
                filename = (
                    sim.cfg.filename + "_spikeStat_" + graphType + "_" + stat + ".png"
                )
            plt.savefig(filename, dpi=dpi)
...
def plotRatePSD(
    ...
    for iplot, subset in enumerate(include):
        ...
        if transformMethod == "morlet":
            ...
            morletSpec = MorletSpec(
                histoCount, Fs, freqmin=minFreq, freqmax=maxFreq, freqstep=stepFreq
            )
            freqs = morletSpec.f
            spec = morletSpec.TFR
    ...
    if overlay:
        ...
        plt.legend(
            fontsize=fontsiz, loc=1
        )  # , bbox_to_anchor=(1.04, 1), loc=2, borderaxespad=0.)
        maxLabelLen = min(10, max([len(str(l)) for l in include]))
        # plt.subplots_adjust(right=(0.9-0.012*maxLabelLen))
...
def calculatefI():
    ...
    sim.allSimData["fI"] = [
        len([spkt for spkt in sim.allSimData["spkt"] if t <= spkt < t + dur])
        / (dur / 1000.0)
        for t in times
    ]
    ...
    if "ISIPeak1" in calculateFeatures:
        ...
        for i in range(len(times)):
            value = (
                (peakTimes[i][1] - peakTimes[i][0]) * dt
                if len(peakTimes[i]) > 1
                else -1
            )
