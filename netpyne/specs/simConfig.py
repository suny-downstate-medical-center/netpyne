...
class SimConfig(object):
    ...
    def __init__(self, simConfigDict=None):
        ...
        self.hParams = Dict(
            {"celsius": 6.3, "v_init": -65.0, "clamp_resist": 0.001}
        )  # parameters of h module
        ...
        self.use_fast_imem = (
            False  # use CVode fast_imem to record membrane voltage via i_membrane_
        )
        ...
        self.includeParamsLabel = (
            True  # include label of param rule that created that cell, conn or stim
        )
        self.gatherOnlySimData = (
            False  # omits gathering of net+cell data thus reducing gatherData time
        )
        ...
        self.pt3dRelativeToCellLocation = (
            True  # Make cell 3d points relative to the cell x,y,z location
        )
        ...
        self.printRunTime = (
            False  # print run time at interval (in sec) specified here (eg. 0.1)
        )
        self.printPopAvgRates = False  # print population avg firing rates after run
        self.printSynsAfterRule = (
            False  # print total of connections after each conn rule is applied
        )
        ...
        self.recordCells = (
            []
        )  # what cells to record traces from (eg. 'all', 5, or 'PYR')
        self.recordTraces = {}  # Dict of traces to record
        self.recordCellsSpikes = (
            -1
        )  # cells to record spike times from (-1 to record from all)
        ...
        self.backupCfgFile = (
            []
        )  # copy cfg file, list with [sourceFile,destFolder] (eg. ['cfg.py', 'backupcfg/'])

        # error checking
        self.checkErrors = False  # whether to validate the input parameters (will be turned off if num processors > 1)
        self.checkErrorsVerbose = (
            False  # whether to print detailed errors during input parameter validation
        )
