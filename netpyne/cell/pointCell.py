...
class PointCell(Cell):
    ...
    def __init__(self, gid, tags, create=True, associateGid=True):
        ...
        if "cellType" in tags and tags["cellType"] in sim.net.params.cellParams:
            tags.update(
                {
                    k: v
                    for k, v in sim.net.params.cellParams[tags["cellType"]].items()
                    if k not in exclude
                }
            )
    ...
    def createNEURONObj(self):
        ...
        try:
            ...
            print(
                "Error creating point process mechanism %s in cell with gid %d"
                % (self.tags["cellModel"], self.gid)
            )
            return
        ...
        if (
            "rate" in self.params
            and isinstance(self.params["rate"], list)
            and len(self.params["rate"]) == 2
        ):
            rand = h.Random()
            rand.Random123(
                sim.hashStr("point_rate"), self.gid, sim.cfg.seeds["stim"]
            )  # initialize randomizer
            self.params["rate"] = rand.uniform(
                self.params["rate"][0], self.params["rate"][1]
            )

        # if rates is list with 2 items generate time-depenedent rates using vector.play()
        if (
            "rates" in self.params
            and isinstance(self.params["rates"], list)
            and len(self.params["rates"]) == 2
        ):
            interval = [
                1000.0 / r if r > 0.0 else 1e10 for r in self.params["rates"][0]
            ]
            self.hVectorIntervals = h.Vector(interval)
            self.hVectorTimes = h.Vector(self.params["rates"][1])
            self.hVectorIntervals.play(
                self.hPointp._ref_interval, self.hVectorTimes
            )  # set continuous = True?
        ...
        if self.tags["cellModel"] == "VecStim":
            ...
            if "interval" in self.params:
                ...
                fixedInterval = np.full(
                    int(((1 + 1.5 * noise) * sim.cfg.duration / interval)),
                    [(1.0 - noise) * interval],
                ...
                if noise == 0.0:
                    ...
                    rand = h.Random()
                    rand.Random123(
                        sim.hashStr("vecstim_spkt"), self.gid, self.params["seed"]
                    )
                    ...
                    if numSpks < 100:
                        ...
                        for i in range(numSpks):
                            vec.setrand(rand)
                            negexpInterval.append(
                                vec.x[0]
                            )  # = np.array(vec)[0:len(fixedInterval)]
                        spkTimes = np.cumsum(
                            fixedInterval + np.array(negexpInterval)
                        ...
                        spkTimes = np.cumsum(fixedInterval + negexpInterval) + (
                            start - interval * (1 - noise)
                        )
            ...
            # spikePattern
                ...
                if self.params["spikePattern"].get("sync", False):
                    rand.Random123(
                        sim.hashStr("vecstim_spikePattern"), self.params["seed"]
                    )
                else:
                    rand.Random123(
                        sim.hashStr("vecstim_spikePattern"),
                        self.gid,
                        self.params["seed"],
                    )
            ...
            if "pulses" in self.params:
                for ipulse, pulse in enumerate(self.params["pulses"]):
                    ...
                    if (
                        "rate" in pulse
                        and isinstance(pulse["rate"], list)
                        and len(pulse["rate"]) == 2
                    ):
                        rand = h.Random()
                        rand.Random123(
                            sim.hashStr("point_rate_pulse" + str(ipulse)),
                            self.gid,
                            sim.cfg.seeds["stim"],
                        )  # initialize randomizer
                    ...
                    if "interval" in pulse:
                        ...
                        print(
                            'Error: Vecstim pulse missing "rate" or "interval" parameter'
                        )
                        return
                    ...
                    if any([x not in pulse for x in ["start", "end"]]):
                        print(
                            'Error: Vecstim pulse missing "start" and/or "end" parameter'
                        )
                        return
                    else:
                        ...
                        # fixed interval of duration (1 - noise)*interval
                        fixedInterval = np.full(
                            int(((1 + 1.5 * noise) * (end - start) / interval)),
                            [(1.0 - noise) * interval],
                        ...
                        if noise == 0.0:
                            ...
                            spkTimes = np.append(
                                spkTimes, pulseSpikes[pulseSpikes <= end]
                            )
                        else:
                            # plus negexp interval of mean duration noise*interval. Note that the most likely negexp interval has duration 0.
                            rand = h.Random()
            ...
            self.hSpkTimes = (
                vec  # store the vector containins spikes to avoid seg fault
            )
    ...
    def associateGid(self, threshold=None):
        ...
        if sim.cfg.createNEURONObj:
            ...
            threshold = (
                threshold if threshold is not None else sim.net.params.defaultThreshold
            )
    ...
    def addConn(self, params, netStimParams=None):
        ...
        weightIndex = params.get(
            "weightIndex"
        )  # note that loop below will overwrite the weights value in weights[i]
        ...
        for i in range(params["synsPerConn"]):
            ...
            if sim.cfg.createNEURONObj:
                if "vref" in self.tags:
                    postTarget = getattr(
                        self.hPointp, "_ref_" + self.tags["vref"]
                    )  #  local point neuron
                else:
                ...
                netcon.weight[weightIndex] = weights[
                    i
                ]  # set Netcon weight - note that this gets over-written in loop
                ...
                if self.conns[-1].get("plast"):
                    self.conns[-1]["dummySec"] = {
                        "hObj": h.Section(name="dummySec", cell=self)
                    }
                    self._addConnPlasticity(
                        self.conns[-1], self.conns[-1]["dummySec"], netcon, weightIndex
                    )
    ...
    def __getattr__(self, name):
        def wrapper(*args, **kwargs):
            ...
            try:
                ...
                if sim.cfg.verbose:
                    print(
                        "Error: Function '%s' not yet implemented for Point Neurons"
                        % name
                    )
