...
-- trying EDSC = LTS cell -- higher firing rate (faster movs), but not sure if sustained without input -- still see oscillations
...
** connectivity
*** Split PMd inputs into 2 subpopulations, each of which only get activated for one target 
- simplest method
