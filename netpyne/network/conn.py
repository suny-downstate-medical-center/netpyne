...
def connectCells(self):
    ...
    if (
        self.params.subConnParams
    ):  # do not create NEURON objs until synapses are distributed based on subConnParams
    ...
    for (
        connParamLabel,
        connParamTemp,
    ) in self.params.connParams.items():  # for each conn rule or parameter set
        ...
        if preCellsTags and postCellsTags:
            ...
            connFunc(
                preCellsTags, postCellsTags, connParam
            )  # call specific conn function

        # check if gap junctions in any of the conn rules
        if not gapJunctions and "gapJunction" in connParam:
            gapJunctions = True
    ...
    if gapJunctions:
        ...
        data[sim.rank] = None
        gather = sim.pc.py_alltoall(
            data
        )  # collect cells data from other nodes (required to generate connections)
        sim.pc.barrier()
        for dataNode in gather:
            if dataNode:
                sim.net.preGapJunctions.extend(dataNode)
    ...
    if self.params.subConnParams:
        ...
        cellsUpdate = [
            ...
            if c.tags.get("cellModel", None) not in ["NetStim", "VecStim"]
        ]
    ...
    if nodeSynapses != nodeConnections:
        print(
            ("  Number of synaptic contacts on node %i: %i " % (sim.rank, nodeSynapses))
        )
    sim.pc.barrier()
    sim.timing("stop", "connectTime")
    if sim.rank == 0 and sim.cfg.timing:
        print(
            ("  Done; cell connection time = %0.2f s." % sim.timingData["connectTime"])
        )
...
def _findPrePostCellsCondition(self, allCellTags, preConds, postConds):
    ...
    for (
        condKey,
        condValue,
    ) in preConds.items():  # Find subset of cells that match presyn criteria
        if condKey in ["x", "y", "z", "xnorm", "ynorm", "znorm"]:
            ...
            if isinstance(condValue, list):
                preCellsTags = {
                    gid: tags
                    for (gid, tags) in preCellsTags.items()
                    if tags.get(condKey, None) in condValue
                }  # dict with pre cell tags
            else:
                preCellsTags = {
                    gid: tags
                    for (gid, tags) in preCellsTags.items()
                    if tags.get(condKey, None) == condValue
                }  # dict with pre cell tags
    ...
    if preCellsTags:  # only check post if there are pre
        ...
        for (
            condKey,
            condValue,
        ) in postConds.items():  # Find subset of cells that match postsyn criteria
            if condKey in ["x", "y", "z", "xnorm", "ynorm", "znorm"]:
                postCellsTags = {
                    gid: tags
...
def _connStrToFunc(self, preCellsTags, postCellsTags, connParam):
    ...
    paramsStrFunc = [
        param
        for param in self.connStringFuncParams
    ...
    dictVars["dist_xnorm"] = lambda preConds, postConds: abs(
        preConds["xnorm"] - postConds["xnorm"]
    )
    dictVars["dist_ynorm"] = lambda preConds, postConds: abs(
        preConds["ynorm"] - postConds["ynorm"]
    )
    dictVars["dist_znorm"] = lambda preConds, postConds: abs(
        preConds["znorm"] - postConds["znorm"]
    )
    ...
    for paramStrFunc in paramsStrFunc:
        ...
        for randmeth in self.stringFuncRandMethods:
            strFunc = strFunc.replace(
                randmeth, "rand." + randmeth
            )  # append rand. to h.Random() methods
        strVars = [
            var
            for var in list(dictVars.keys())
            if var in strFunc and var + "norm" not in strFunc
        ]  # get list of variables used (eg. post_ynorm or dist_xyz)
        lambdaStr = (
            "lambda " + ",".join(strVars) + ": " + strFunc
        ...
        if paramStrFunc in ["probability"]:
            ...
            connParam[paramStrFunc + "FuncVars"] = {
                strVar: dictVars[strVar] for strVar in strVars
            }
...
# -----------------------------------------------------------------------------
def _disynapticBiasProb(
    self, origProbability, bias, prePreGids, postPreGids, disynCounter, maxImbalance=10
):
    ...
    if not set(prePreGids).isdisjoint(postPreGids) and disynCounter < maxImbalance:
        ...
        probability = max(
            origProbability - (min(origProbability + bias, 1.0) - origProbability), 0.0
        )
        disynCounter -= 1
...
def fullConn(self, preCellsTags, postCellsTags, connParam):
    ...
    paramsStrFunc = [
        param
        for param in [p + "Func" for p in self.connStringFuncParams]
...
def generateRandsPrePost(self, pre, post):
    ...
    self.rand.Random123(
        sim.hashList(sortedPre), sim.hashList(sortedPost), sim.cfg.seeds["conn"]
    )  #
...
def probConn(self, preCellsTags, postCellsTags, connParam):
    ...
    for paramStrFunc in paramsStrFunc:
        ...
        funcKeys[paramStrFunc] = [
            key
            for key in connParam[paramStrFunc + "Vars"]
    ...
    if isinstance(connParam.get("disynapticBias", None), Number):
        ...
        probMatrix = {
            (preCellGid, postCellGid): connParam["probabilityFunc"][
                preCellGid, postCellGid
            ]
        ...
        for preCellGid, postCellGid in connGids:
            for (
                paramStrFunc
            ) in paramsStrFunc:  # call lambda functions to get weight func args
                connParam[paramStrFunc + "Args"] = {
                    k: v
                    if isinstance(v, Number)
                    else v(preCellsTags[preCellGid], postCellsTags[postCellGid])
                    for k, v in connParam[paramStrFunc + "Vars"].items()
                }
            self._addCellConn(connParam, preCellGid, postCellGid)  # add connection
    ...
    # standard probabilistic conenctions
        ...
        for (
            postCellGid,
            postCellTags,
        ) in postCellsTags.items():  # for each postsyn cell  # for each postsyn cell
            if postCellGid in self.gid2lid:  # check if postsyn is in this node
                for (
                    preCellGid,
                    preCellTags,
                ) in preCellsTags.items():  # for each presyn cell
                    probability = (
                    ...
                    if probability >= allRands[preCellGid, postCellGid]:
                        for (
                            ...
                            for funcKey in funcKeys[paramStrFunc]:
                                connParam[paramStrFunc + "Args"][funcKey] = connParam[
                                    paramStrFunc + "Vars"
                                ][funcKey](preCellTags, postCellTags)
                        ...
                        self._addCellConn(
                            connParam, preCellGid, postCellGid
                        )  # add connection
...
def convConn(self, preCellsTags, postCellsTags, connParam):
    ...
    if sim.cfg.verbose:
        print(
            "Generating set of convergent connections (rule: %s) ..."
            % (connParam["label"])
        )
    ...
    for postCellGid, postCellTags in postCellsTags.items():  # for each postsyn cell
        if postCellGid in self.gid2lid:  # check if postsyn is in this node
            ...
            self.rand.Random123(
                hashPreCells, postCellGid, sim.cfg.seeds["conn"]
            )  # init randomizer
            randSample = self.randUniqueInt(
                self.rand, convergence + 1, 0, len(preCellsTags) - 1
            )
...
def divConn(self, preCellsTags, postCellsTags, connParam):
    ...
    for preCellGid, preCellTags in preCellsTags.items():  # for each presyn cell
        divergence = (
            connParam["divergenceFunc"][preCellGid]
            if "divergenceFunc" in connParam
            else connParam["divergence"]
        )  # num of presyn conns / postsyn cell
        divergence = max(min(int(round(divergence)), len(postCellsTags) - 1), 0)
        self.rand.Random123(
            hashPostCells, preCellGid, sim.cfg.seeds["conn"]
        )  # init randomizer
        randSample = self.randUniqueInt(
            self.rand, divergence + 1, 0, len(postCellsTags) - 1
        )
            ...
            (orderedPreGids[preId], orderedPostGids[postId]): connParam[paramStrFunc](
                **{
                    ...
                    if isinstance(v, Number)
                        ...
                        preCellsTags[orderedPreGids[preId]],
                        postCellsTags[orderedPostGids[postId]],
                    )
                    for k, v in connParam[paramStrFunc + "Vars"].items()
                }
            )
    ...
    for param in paramStrFunc:
        if param + "List" in connParam:
            ...
            if not randSeeded and "rand" in connParam[param + "FuncArgs"]:
                ...
                randSeeded = True
            finalParam[param] = connParam[param + "Func"](
                **connParam[param + "FuncArgs"]
            )
        else:
    ...
    for i, synMech in enumerate(connParam.get("synMech")):
        ...
        for param in paramPerSynMech:
            ...
            if len(connParam["synMech"]) > 1:
                if isinstance(
                    finalParam.get(param), list
                ):  # get weight from list for each synMech
                    finalParam[param + "SynMech"] = finalParam[param][i]
                elif (
                    "synMech" + param.title() + "Factor" in connParam
                ):  # adapt weight for each synMech
                    finalParam[param + "SynMech"] = (
                        finalParam[param]
                        * connParam["synMech" + param.title() + "Factor"][i]
                    )
