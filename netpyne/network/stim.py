...
def addStims(self):
    ...
    if self.params.stimSourceParams and self.params.stimTargetParams:
        ...
        for (
            targetLabel,
            target,
        ) in self.params.stimTargetParams.items():  # for each target parameter set
            if "sec" not in target:
                target[
                    "sec"
                ] = None  # if section not specified, make None (will be assigned to first section in cell)
            ...
            for condKey, condValue in target[
                "conds"
            ].items():  # Find subset of cells that match postsyn criteria
            ...
            if "cellList" in target["conds"]:
                ...
                postCellsTags = {
                    gid: tags for (gid, tags) in postCellsTags.items() if gid in gidList
                }

            # initialize randomizer in case used in string-based function (see issue #89 for more details)
            self.rand.Random123(
                sim.hashStr("stim_" + source["type"]),
            ...
            for postCellGid in postCellsTags:  # for each postsyn cell
                if (
                    ...
                    params["sec"] = (
                        strParams["secList"][postCellGid]
                        if "secList" in strParams
                        else target["sec"]
                    )
                    params["loc"] = (
                        strParams["locList"][postCellGid]
                        if "locList" in strParams
                        else target["loc"]
                    )

                    if (
                        source["type"] == "NetStim"
                    ):  # for NetStims add weight+delay or default values
                        params["weight"] = (
                        ...
                        for p in ["Weight", "Delay", "loc"]:
                            if "synMech" + p + "Factor" in target:
                                params["synMech" + p + "Factor"] = target.get(
                                    "synMech" + p + "Factor"
                                )

                    if (
                        "originalFormat" in source
                        and source["originalFormat"] == "NeuroML2"
                    ):
                        if "weight" in target:
                    ...
                    if source["type"] == "NetStim":
                        self._addCellStim(
                            params, postCell
                        )  # call method to add connections (sort out synMechs first)
                    else:
                        postCell.addStim(params)  # call cell method to add connection

    print(
        (
            "  Number of stims on node %i: %i "
            % (sim.rank, sum([len(cell.stims) for cell in self.cells]))
        )
    )
    sim.pc.barrier()
    ...
    if sim.rank == 0 and sim.cfg.timing:
        print(
            (
                "  Done; cell stims creation time = %0.2f s."
                % sim.timingData["stimsTime"]
            )
        )
...
def _stimStrToFunc(self, postCellsTags, sourceParams, targetParams):
    ...
    paramsStrFunc = [
        ...
        if param in params
        and isinstance(params[param], basestring)
        and params[param] not in ["variable"]
    ]
