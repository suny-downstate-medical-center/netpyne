...
def cp(obj, verbose=True, die=True):
    ...
    try:
        ...
        output = obj
        errormsg = (
            "Warning: could not perform shallow copy, returning original object: %s"
            % str(E)
        )
        if die:
...
def dcp(obj, verbose=True, die=False):
    ...
    try:
        ...
        output = cp(obj)
        errormsg = (
            "Warning: could not perform deep copy, performing shallow instead: %s"
            % str(E)
        )
        if die:
            raise Exception(errormsg)
    ...
    for i in range(n):
        ...
        try:
            if x == 0:
                ...
                x > (10 ** sigfigs) and not SI and keepints
            ):  # e.g. x = 23432.23, sigfigs=3, output is 23432
                ...
                digits = int(
                    abs(magnitude)
                    + max(0, sigfigs - max(0, magnitude) - 1)
                    + 1
                    + (x < 0)
                    + (abs(x) < 1)
                ...
                if (
                    sep
                ):  # To insert separators in the right place, have to convert back to a number
                    if decimals > 0:
